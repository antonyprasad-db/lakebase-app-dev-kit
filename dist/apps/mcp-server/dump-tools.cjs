...
var storyPlanJson = (tdd, f, s) => (0, import_node_path2.join)(storyResolved(tdd, f, s), "plan.json");
...
function verifyHooks(projectDir) {
  ...
  return {
    postCheckout: fs4.existsSync(path2.join(hooksDir, "post-checkout")),
    prepareCommitMsg: fs4.existsSync(path2.join(hooksDir, "prepare-commit-msg")),
    prePush: fs4.existsSync(path2.join(hooksDir, "pre-push"))
  };
}
function verifyWorkflows(projectDir) {
  const wfDir = path2.join(projectDir, ".github", "workflows");
  return {
    pr: fs4.existsSync(path2.join(wfDir, "pr.yml")),
    merge: fs4.existsSync(path2.join(wfDir, "merge.yml"))
  };
}
...
function warmAndVerifyKit(projectDir, timeoutMs = 18e4) {
  ...
  const warm = (0, import_node_child_process4.spawnSync)("bash", [lk, "--warm"], { cwd: projectDir, encoding: "utf-8", timeout: timeoutMs });
  if (warm.status !== 0) {
    return { ok: false, reason: lastLines(warm.stderr) || `lk --warm exited ${warm.status ?? "(killed)"}` };
  }
  const verify = (0, import_node_child_process4.spawnSync)("bash", [lk, "lakebase-schema-diff", "--help"], {
    cwd: projectDir,
    encoding: "utf-8",
    timeout: 3e4,
...
function extractZipToDir(zipBuffer, targetDir) {
  ...
  const sourceDir = entries.length === 1 && fs7.statSync(path5.join(tempDir, entries[0])).isDirectory() ? path5.join(tempDir, entries[0]) : tempDir;
...
function clearScaffoldArtifacts(targetDir) {
  ...
  for (const entry of fs9.readdirSync(targetDir)) {
    ...
    fs9.rmSync(path6.join(targetDir, entry), { recursive: true, force: true });
  }
}
...
function findTemplatesDir2() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (fs10.existsSync(path7.join(candidate, "common", ".gitignore.base"))) {
...
function clientTemplateDir(opts) {
  return path8.join(templatesRoot(opts), "client");
}
function listFilesRelative(dir, relPrefix = "") {
  const out = [];
  for (const entry of fs11.readdirSync(dir)) {
    const p = path8.join(dir, entry);
    const rel = relPrefix ? path8.join(relPrefix, entry) : entry;
...
function deployClientProject(targetDir, projectName, opts) {
  ...
  const destDir = path8.join(targetDir, "client");
  copyDirSubstituted(src, destDir, { projectName });
  return listFilesRelative(destDir).map((r) => path8.join("client", r));
}
async function deployClaudeCommands(targetDir, opts) {
  ...
  for (const entry of fs11.readdirSync(src)) {
    ...
    const before = fs11.readFileSync(path8.join(src, entry), "utf-8");
...
async function deployClaudeAgents(targetDir, opts) {
  ...
  for (const entry of fs11.readdirSync(src)) {
    ...
    fs11.copyFileSync(path8.join(src, entry), destPath);
    written.push(relDest);
  }
  return { written, skipped };
}
...
function kitVersion(opts) {
  try {
    const kitRoot = path8.dirname(path8.dirname(templatesRoot(opts)));
    const raw = fs11.readFileSync(path8.join(kitRoot, "package.json"), "utf-8");
...
async function installHooks(targetDir) {
  ...
  if (!fs11.existsSync(path8.join(targetDir, ".git"))) {
...
async function deployEnvExample(targetDir, args = {}) {
  fs11.writeFileSync(path8.join(targetDir, ".env.example"), renderEnvFromTemplate(args));
}
async function deployEnv(targetDir, args = {}) {
  fs11.writeFileSync(path8.join(targetDir, ".env"), renderEnvFromTemplate(args));
}
...
async function deployVscodeSettings(targetDir, opts) {
  ...
  fs11.mkdirSync(destDir, { recursive: true });
  fs11.copyFileSync(src, path8.join(destDir, "settings.json"));
}
async function deployGitignore(targetDir, language = "java", opts) {
  const base = fs11.readFileSync(path8.join(commonDir(opts), ".gitignore.base"), "utf-8");
  ...
  fs11.writeFileSync(path8.join(targetDir, ".gitignore"), base + "\n" + extra);
}
...
function findTemplatesDir4() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (fs12.existsSync(path9.join(candidate, "common", ".gitignore.base"))) {
...
function writePlaywrightTemplates(args) {
  ...
  for (const rel of args.files ?? PLAYWRIGHT_TEMPLATE_FILES) {
    ...
    fs12.mkdirSync(path9.dirname(to), { recursive: true });
...
function enableE2eForProject(args) {
  if (args.clientOwnsE2e) {
    const isPython = args.language === "python" || fs13.existsSync(path10.join(args.projectDir, "pyproject.toml"));
...
function stopRunner(projectName) {
  ...
  try {
    fs15.mkdirSync(path12.join(dir, "_diag", "pages"), { recursive: true });
  } catch {
  }
}
function resetRunnerConfig(dir, projectName) {
  ...
  for (const f of stateFiles) {
    try {
      fs15.unlinkSync(path12.join(dir, f));
    } catch {
    }
  }
...
async function setupRunner(args) {
  ...
  fs15.mkdirSync(dir, { recursive: true });
  if (!fs15.existsSync(path12.join(dir, "config.sh"))) {
  ...
  if (child.pid) {
    fs15.writeFileSync(path12.join(dir, ".pid"), String(child.pid));
  }
      ...
      if (enableSftdd) {
        ...
        if (kitRef) {
          try {
            ...
            fs17.writeFileSync(path14.join(dir, "kit-ref"), `${kitRef}
`, "utf8");
          } catch (err) {
...
function detectLanguageAt(dir) {
  if (fs18.existsSync(path15.join(dir, "pom.xml"))) {
    const kotlinDir = path15.join(dir, "src", "main", "kotlin");
    if (fs18.existsSync(kotlinDir)) {
      return "kotlin";
    }
    try {
      const pom = fs18.readFileSync(path15.join(dir, "pom.xml"), "utf-8");
  ...
  if (fs18.existsSync(path15.join(dir, "pyproject.toml")) || fs18.existsSync(path15.join(dir, "requirements.txt")) || fs18.existsSync(path15.join(dir, "alembic.ini"))) {
    return "python";
  }
  if (fs18.existsSync(path15.join(dir, "package.json"))) {
    return "nodejs";
  }
  return "unknown";
}
...
function spawnAlembic(projectDir, args, dsn) {
  return new Promise((resolve3, reject) => {
    ...
    const env = { ...process.env };
    env.PYTHONPATH = [projectDir, process.env.PYTHONPATH].filter(Boolean).join(path16.delimiter);
...
var AlembicAdapter = {
   ...
   */
  detect(projectDir) {
    if (fs20.existsSync(path17.join(projectDir, "alembic.ini"))) return true;
    if (fs20.existsSync(path17.join(projectDir, "migrations", "env.py"))) return true;
    if (fs20.existsSync(path17.join(projectDir, "alembic", "env.py"))) return true;
    return false;
  },
  async apply(args) {
...
var FlywayAdapter = {
  ...
  detect(projectDir) {
    return fs21.existsSync(path19.join(projectDir, "pom.xml"));
  },
  async apply(args) {
...
var KnexAdapter = {
   ...
   */
  detect(projectDir) {
    return KNEXFILE_VARIANTS2.some((name) => fs23.existsSync(path21.join(projectDir, name)));
  },
  async apply(args) {
...
function gitHasLocalBranch(cwd, branch) {
  try {
    (0, import_node_child_process8.execFileSync)("git", ["rev-parse", "--verify", "--quiet", `refs/heads/${branch}`], {
      cwd,
      stdio: "ignore",
      timeout: KIT_TIMEOUTS.gitDefault
    });
...
function gitDeleteLocalBranch(cwd, branch, force = true) {
  (0, import_node_child_process8.execFileSync)("git", ["branch", force ? "-D" : "-d", branch], {
    cwd,
    stdio: ["ignore", "pipe", "pipe"],
    timeout: KIT_TIMEOUTS.gitDefault
  });
...
function gitDeleteRemoteBranch(cwd, remote, branch) {
  (0, import_node_child_process8.execFileSync)("git", ["push", remote, "--delete", branch], {
    cwd,
    stdio: ["ignore", "pipe", "pipe"],
...
function readDriveContext(sftddDir, featureId, projectDir) {
  ...
  const proj = projectDir ?? path25.dirname(sftddDir);
  let scmState;
  try {
...
function readJsonIfExists(path28) {
  if (!(0, import_fs8.existsSync)(path28)) return null;
  return JSON.parse((0, import_fs8.readFileSync)(path28, "utf8"));
}
...
function readSelectionLogRecent(sftddDir, limit) {
  const path28 = (0, import_path7.join)(sftddDir, "selection-log.md");
  if (!(0, import_fs8.existsSync)(path28)) return [];
  const text = (0, import_fs8.readFileSync)(path28, "utf8");
  const entries = [];
...
function detectWorkflowDrift(args) {
  const projectWorkflowsDir = path26.join(
    args.projectDir,
    ".github",
    "workflows"
  );
  const here = path26.dirname(new URL(importMetaUrl).pathname);
  const kitWorkflowsDir = args.kitDir ? path26.join(
    args.kitDir,
    "templates",
    "project",
    "common",
  ...
  for (const name of templateFiles) {
    seen.add(name);
    const projectPath2 = path26.join(projectWorkflowsDir, name);
    const templatePath = path26.join(kitWorkflowsDir, name);
...
function readKitVersion(kitWorkflowsDir) {
  ...
  try {
    const raw = fs31.readFileSync(path26.join(dir, "package.json"), "utf-8");
    const pkg = JSON.parse(raw);
    return typeof pkg.version === "string" ? pkg.version : "unknown";
  } catch {
    return "unknown";
...
function checkEnv(projectDir) {
  ...
  if (Object.keys(env).length === 0) {
    return {
      ...
      detail: { projectDir, envPath: path27.join(projectDir, ".env") },
