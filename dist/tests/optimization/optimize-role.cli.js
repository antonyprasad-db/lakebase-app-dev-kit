...
import { mkdirSync as mkdirSync20, writeFileSync as writeFileSync19, readFileSync as readFileSync32, existsSync as existsSync34 } from "fs";
import { join as join34, dirname as dirname16 } from "path";
...
init_esm_shims();
import { mkdtempSync, mkdirSync as mkdirSync19, rmSync as rmSync8, existsSync as existsSync32, readFileSync as readFileSync30, readdirSync as readdirSync18 } from "fs";
import { tmpdir } from "os";
import { join as join30, relative as relative3 } from "path";

// consort/orchestrator/runners/manifest-runner.ts
init_esm_shims();
import { readFileSync as readFileSync29, existsSync as existsSync30 } from "fs";
...
function navigatorTestsAuthored(producedPath) {
  ...
  return walk(producedPath) ? { ok: true, violations: [] } : { ok: false, violations: [`navigator RED tests/ tree at ${producedPath} has no test file (.py/.ts/.tsx)`] };
}
function driverCodePresent(producedPath) {
  ...
  return walk(producedPath) ? { ok: true, violations: [] } : { ok: false, violations: [`driver GREEN app/ tree at ${producedPath} has no source file (.py/.ts/.tsx)`] };
}
function assessMarkerWritten(producedPath) {
  const sup = join4(producedPath, "superseded-tests.json");
  const reg = join4(producedPath, "regression-assessment.json");
...
function supersededTestsJson(tdd, feature, story, ac) {
  return join6(cycleDir(tdd, feature, story, ac), "superseded-tests.json");
}
...
function greenFailureJson(tdd, feature, story, ac) {
  return join6(cycleDir(tdd, feature, story, ac), "green-failure.json");
}
...
import { readFileSync as readFileSync14, writeFileSync as writeFileSync8, existsSync as existsSync15 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync13, mkdirSync as mkdirSync8, cpSync as cpSync2, rmSync as rmSync2, readdirSync as readdirSync7, statSync as statSync4 } from "fs";
...
init_esm_shims();
import { readFileSync as readFileSync13, writeFileSync as writeFileSync7, existsSync as existsSync14, mkdirSync as mkdirSync9, cpSync as cpSync3 } from "fs";
  ...
  return {
    async invoke(invocation) {
      ...
      for (const seed of opts.seeds) {
        const src = join14(opts.corpusRoot, seed.from);
        if (!existsSync14(src)) {
          throw new Error(
        ...
        const dst = join14(invocation.workspaceDir, seed.to);
        if (seed.kind === "tree") {
          mkdirSync9(dst, { recursive: true });
          cpSync3(src, dst, { recursive: true, force: true, filter: codeTreeFilter(src) });
        } else {
          mkdirSync9(dirname8(dst), { recursive: true });
          writeFileSync7(dst, readFileSync13(src, "utf8"));
        }
      ...
      const logPath = join14(invocation.workspaceDir, "agent-log.jsonl");
      const prior = existsSync14(logPath) ? readFileSync13(logPath, "utf8") : "";
  ...
  return {
    async invoke(invocation) {
      for (const [filename, contents] of Object.entries(outputs)) {
        writeFileSync8(join15(invocation.workspaceDir, filename), contents);
      }
      const logPath = join15(invocation.workspaceDir, "agent-log.jsonl");
      const prior = existsSync15(logPath) ? readFileSync14(logPath, "utf8") : "";
...
init_esm_shims();
import { existsSync as existsSync24, readFileSync as readFileSync24, readdirSync as readdirSync15, statSync as statSync9, writeFileSync as writeFileSync14, mkdirSync as mkdirSync14, rmSync as rmSync6 } from "fs";
...
import { existsSync as existsSync19, mkdirSync as mkdirSync12, readFileSync as readFileSync19, rmSync as rmSync4, writeFileSync as writeFileSync12 } from "fs";
...
function storyTestListFingerprint(consortDir, featureId, story_id) {
  ...
  try {
    return createHash("sha1").update(readFileSync15(f)).digest("hex");
  } catch {
    return "";
  }
}
...
init_esm_shims();
import { existsSync as existsSync20, readFileSync as readFileSync20, readdirSync as readdirSync12 } from "fs";
import { join as join20 } from "path";

// consort/architecture/contract-clean.ts
init_esm_shims();
import { existsSync as existsSync21, readFileSync as readFileSync21, readdirSync as readdirSync13, statSync as statSync7 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync23, readFileSync as readFileSync23, readdirSync as readdirSync14, statSync as statSync8 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync25, readFileSync as readFileSync25, writeFileSync as writeFileSync15, mkdirSync as mkdirSync15, rmSync as rmSync7 } from "fs";
var SMELL_FOR_OWNER = {
...
init_esm_shims();
import { existsSync as existsSync26, readFileSync as readFileSync26, writeFileSync as writeFileSync16, mkdirSync as mkdirSync16, readdirSync as readdirSync16 } from "fs";
function uniq(xs) {
...
init_esm_shims();
import { readFileSync as readFileSync28, writeFileSync as writeFileSync17, existsSync as existsSync29, appendFileSync, mkdirSync as mkdirSync17 } from "fs";
...
function formatAgentReport(args) {
  const logPath = join27(args.workspaceDir, args.logFile ?? "agent-log.jsonl");
  let reportJson;
  let source;
  if (args.reportText !== void 0) {
    ...
    const reportPath = join27(args.workspaceDir, args.reportFile ?? ".agent-report.json");
    source = reportPath;
    ...
    reportJson = readFileSync28(reportPath, "utf8");
  }
  let raw;
  try {
  ...
  if (existsSync29(logPath)) {
    ...
    mkdirSync17(dirname14(logPath), { recursive: true });
    writeFileSync17(logPath, payload);
  }
...
init_esm_shims();
import { mkdirSync as mkdirSync18, cpSync as cpSync4, existsSync as existsSync31 } from "fs";
import { join as join29, dirname as dirname15 } from "path";
function layDownKitAgents(workspaceDir, kitDir = process.cwd()) {
  const src = join29(kitDir, "skills", "consort", "agents");
  if (!existsSync31(src)) throw new Error(`layDownKitAgents: kit agents dir not found at ${src}`);
  const dest = join29(workspaceDir, ".claude", "agents");
  mkdirSync18(dest, { recursive: true });
  cpSync4(src, dest, { recursive: true });
}

// consort/orchestrator/scenarios/integration-chain.ts
async function runIntegrationChain(config) {
  const manifests = loadStepManifests(config.manifestDir);
  const workspaceDir = mkdtempSync(join30(tmpdir(), "integration-chain-"));
  mkdirSync19(join30(workspaceDir, ARTIFACT_ROOT), { recursive: true });
  ...
  const runnerDeps = {
    ...
    cfg: {
      ...
      consortDir: join30(workspaceDir, ARTIFACT_ROOT),
  ...
  try {
    ...
    const producedArtifacts = snapshotTree(join30(workspaceDir, ARTIFACT_ROOT), workspaceDir);
    for (const root of config.extraSnapshotRoots ?? []) {
      Object.assign(producedArtifacts, snapshotTree(join30(workspaceDir, root), workspaceDir));
    }
...
function snapshotTree(root, relTo) {
  const out = {};
  if (!existsSync32(root)) return out;
  const walk = (dir) => {
    for (const entry of readdirSync18(dir, { withFileTypes: true })) {
      const abs = join30(dir, entry.name);
      if (entry.isDirectory()) walk(abs);
      else if (entry.isFile()) out[relative3(relTo, abs)] = readFileSync30(abs, "utf8");
    }
  };
  walk(root);
  return out;
...
async function runRoleChainLive(chain, opts = {}) {
  ...
  const { turns, producedArtifacts } = await runIntegrationChain({
    manifestDir: join31(kit, MANIFESTS_REL, chain.dir),
    intakeDir: join31(kit, INTAKE_REL),
    feature: FEATURE,
    start: PO_SEED,
...
async function runOptimizeRole(args) {
  ...
  const runDir = args.telemetryDir ?? join34(process.cwd(), ".role-telemetry", `sweep-${args.role}-${runStamp()}`);
  mkdirSync20(runDir, { recursive: true });
  ...
  writeFileSync19(join34(runDir, "report.txt"), formatRoleSweepReport(report) + "\n");
...
function readReference(chain, _role) {
  const p = join34(process.cwd(), INTAKE_REL, chain.referenceFile ?? chain.outputFile);
  return existsSync34(p) ? readFileSync32(p, "utf8") : void 0;
}
function persistTrial(runDir, chain, baseModel, trial) {
  const dir = join34(runDir, trial.candidateId);
  mkdirSync20(dir, { recursive: true });
  if (trial.telemetry) writeFileSync19(join34(dir, "telemetry.json"), JSON.stringify(trial.telemetry, null, 2) + "\n");
  for (const [rel, contents] of Object.entries(trial.producedArtifacts ?? {})) {
    const dest = join34(dir, "artifacts", rel);
    mkdirSync20(dirname16(dest), { recursive: true });
    writeFileSync19(dest, contents);
  }
  const replay = {
    role: chain.dir,
  ...
  writeFileSync19(join34(dir, "replay.json"), JSON.stringify(replay, null, 2) + "\n");
}
