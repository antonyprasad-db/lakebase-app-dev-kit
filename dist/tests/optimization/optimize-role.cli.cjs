    ...
    async invoke(invocation) {
      ...
      for (const seed of opts.seeds) {
        ...
        if (seed.kind === "tree") {
          ...
          (0, import_node_fs5.mkdirSync)((0, import_node_path7.dirname)(dst), { recursive: true });
  ...
  return {
    async invoke(invocation) {
      for (const [filename, contents] of Object.entries(outputs)) {
        (0, import_node_fs6.writeFileSync)((0, import_node_path8.join)(invocation.workspaceDir, filename), contents);
      }
...
function readSmellsLog(consortDir) {
  const file = (0, import_path4.join)(consortDir, "smells.json");
...
function storyTestListFingerprint(consortDir, featureId, story_id) {
  ...
  try {
    return (0, import_crypto.createHash)("sha1").update((0, import_fs3.readFileSync)(f)).digest("hex");
  } catch {
    return "";
  }
}
...
function storyDeployVerified(consortDir, featureId, storyId) {
  ...
  return deployEvidencePasses(readDeployEvidence((0, import_node_path10.join)(fdir, "stories", storyId, "deploy-evidence.json")));
}
...
function storyCycles(consortDir, featureId, story) {
  const base = (0, import_path5.join)(cyclesRootDir(consortDir), featureId, story);
  ...
  for (const acDir of (0, import_fs4.readdirSync)(base)) {
    ...
    for (const f of (0, import_fs4.readdirSync)(dir)) {
      ...
      try {
        out.push(JSON.parse((0, import_fs4.readFileSync)((0, import_path5.join)(dir, f), "utf8")));
      } catch {
      }
    }
  }
...
async function execute(step, ctx, deps) {
  ...
  for (const spec of outputSpecs) {
    ...
    const abs = producedPaths.find((p) => p.endsWith(rel)) ?? (0, import_node_path14.join)(root, rel);
...
function formatAgentReport(args) {
  const logPath = (0, import_node_path15.join)(args.workspaceDir, args.logFile ?? "agent-log.jsonl");
  let reportJson;
  let source;
  if (args.reportText !== void 0) {
    ...
    const reportPath = (0, import_node_path15.join)(args.workspaceDir, args.reportFile ?? ".agent-report.json");
    source = reportPath;
  ...
  if ((0, import_node_fs13.existsSync)(logPath)) {
    ...
    (0, import_node_fs13.mkdirSync)((0, import_node_path15.dirname)(logPath), { recursive: true });
...
function resolveInputsFromWorkspace(manifest, workspaceDir, featureId, action) {
  ...
  for (const input of manifest.inputs) {
    ...
    const p = (0, import_node_path16.join)(workspaceDir, file);
...
function layDownKitAgents(workspaceDir, kitDir = process.cwd()) {
  const src = (0, import_node_path17.join)(kitDir, "skills", "consort", "agents");
  if (!(0, import_node_fs15.existsSync)(src)) throw new Error(`layDownKitAgents: kit agents dir not found at ${src}`);
  const dest = (0, import_node_path17.join)(workspaceDir, ".claude", "agents");
...
async function runIntegrationChain(config) {
  ...
  const workspaceDir = (0, import_node_fs16.mkdtempSync)((0, import_node_path18.join)((0, import_node_os.tmpdir)(), "integration-chain-"));
  (0, import_node_fs16.mkdirSync)((0, import_node_path18.join)(workspaceDir, ARTIFACT_ROOT), { recursive: true });
  ...
  const runnerDeps = {
    ...
    cfg: {
      ...
      consortDir: (0, import_node_path18.join)(workspaceDir, ARTIFACT_ROOT),
  ...
  try {
    ...
    const producedArtifacts = snapshotTree((0, import_node_path18.join)(workspaceDir, ARTIFACT_ROOT), workspaceDir);
    for (const root of config.extraSnapshotRoots ?? []) {
      Object.assign(producedArtifacts, snapshotTree((0, import_node_path18.join)(workspaceDir, root), workspaceDir));
    }
...
function snapshotTree(root, relTo) {
  ...
  const walk = (dir) => {
    for (const entry of (0, import_node_fs16.readdirSync)(dir, { withFileTypes: true })) {
      ...
      else if (entry.isFile()) out[(0, import_node_path18.relative)(relTo, abs)] = (0, import_node_fs16.readFileSync)(abs, "utf8");
    }
  };
  walk(root);
  return out;
...
async function runRoleChainLive(chain, opts = {}) {
  ...
  const { turns, producedArtifacts } = await runIntegrationChain({
    manifestDir: (0, import_node_path19.join)(kit, MANIFESTS_REL, chain.dir),
    intakeDir: (0, import_node_path19.join)(kit, INTAKE_REL),
    feature: FEATURE,
    start: PO_SEED,
...
async function runOptimizeRole(args) {
  ...
  const runDir = args.telemetryDir ?? (0, import_node_path22.join)(process.cwd(), ".role-telemetry", `sweep-${args.role}-${runStamp()}`);
  (0, import_node_fs19.mkdirSync)(runDir, { recursive: true });
  ...
  (0, import_node_fs19.writeFileSync)((0, import_node_path22.join)(runDir, "report.txt"), formatRoleSweepReport(report) + "\n");
...
function readReference(chain, _role) {
  const p = (0, import_node_path22.join)(process.cwd(), INTAKE_REL, chain.referenceFile ?? chain.outputFile);
  return (0, import_node_fs19.existsSync)(p) ? (0, import_node_fs19.readFileSync)(p, "utf8") : void 0;
}
function persistTrial(runDir, chain, baseModel, trial) {
  const dir = (0, import_node_path22.join)(runDir, trial.candidateId);
  (0, import_node_fs19.mkdirSync)(dir, { recursive: true });
  if (trial.telemetry) (0, import_node_fs19.writeFileSync)((0, import_node_path22.join)(dir, "telemetry.json"), JSON.stringify(trial.telemetry, null, 2) + "\n");
  for (const [rel, contents] of Object.entries(trial.producedArtifacts ?? {})) {
    const dest = (0, import_node_path22.join)(dir, "artifacts", rel);
    (0, import_node_fs19.mkdirSync)((0, import_node_path22.dirname)(dest), { recursive: true });
    (0, import_node_fs19.writeFileSync)(dest, contents);
  }
  const replay = {
    role: chain.dir,
  ...
  (0, import_node_fs19.writeFileSync)((0, import_node_path22.join)(dir, "replay.json"), JSON.stringify(replay, null, 2) + "\n");
}
