...
__export(scripts_exports, {
  ...
  GitHubRunnerError: () => GitHubRunnerError,
  GitHubSecretsError: () => GitHubSecretsError,
  ...
  getRunnerIdByName: () => getRunnerIdByName,
  ...
  listRepoRunners: () => listRepoRunners,
  ...
  mergePairedPullRequest: () => mergePairedPullRequest,
  ...
  tryGhAuthToken: () => tryGhAuthToken,
  tryVsCodeSession: () => tryVsCodeSession,
...
function resolveProfileForHostSync(host, timeoutMs = KIT_TIMEOUTS.cliDefault) {
  ...
  let out;
  try {
    out = (0, import_node_child_process2.execFileSync)("databricks", ["auth", "profiles", "-o", "json"], {
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
      timeout: timeoutMs
    });
...
function runDatabricksSync(args, opts = {}) {
  const { argv, env, profile } = buildInvocation(args, opts);
  try {
    return (0, import_node_child_process3.execFileSync)("databricks", argv, {
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
      env,
      timeout: opts.timeout ?? KIT_TIMEOUTS.cliDefault
...
function gitMergeBranch(cwd, branch) {
  (0, import_node_child_process4.execFileSync)("git", ["merge", "--no-edit", branch], {
    cwd,
    stdio: ["ignore", "pipe", "pipe"],
    timeout: KIT_TIMEOUTS.gitDefault
  });
...
async function commitAllIfChanged(args) {
  ...
  for (const inc of args.include ?? []) {
    if ((0, import_fs3.existsSync)((0, import_path4.join)(args.cwd, inc))) {
...
function isCliEntry(importMetaUrl2) {
  ...
  let moduleResolved;
  try {
    invokedResolved = (0, import_node_fs2.realpathSync)(invokedRaw);
  } catch {
    return false;
  }
  try {
    moduleResolved = (0, import_node_fs2.realpathSync)((0, import_node_url6.fileURLToPath)(importMetaUrl2));
  } catch {
    return false;
  }
