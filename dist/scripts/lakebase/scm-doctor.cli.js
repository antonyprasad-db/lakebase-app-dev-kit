...
function detectLanguageAt(dir) {
  if (fs5.existsSync(path5.join(dir, "pom.xml"))) {
    const kotlinDir = path5.join(dir, "src", "main", "kotlin");
    if (fs5.existsSync(kotlinDir)) {
      return "kotlin";
    }
    try {
      const pom = fs5.readFileSync(path5.join(dir, "pom.xml"), "utf-8");
  ...
  if (fs5.existsSync(path5.join(dir, "pyproject.toml")) || fs5.existsSync(path5.join(dir, "requirements.txt")) || fs5.existsSync(path5.join(dir, "alembic.ini"))) {
    return "python";
  }
  if (fs5.existsSync(path5.join(dir, "package.json"))) {
    return "nodejs";
  }
  return "unknown";
}
...
function listAlembicMigrations(projectDir) {
  ...
  const dir = candidates.find((p) => fs11.existsSync(p));
  if (!dir) return [];
  const files = fs11.readdirSync(dir).filter((f) => f.endsWith(".py") && !f.startsWith("__"));
  return files.map((filename) => {
    const stem = filename.replace(/\.py$/, "");
    const sep2 = stem.indexOf("_");
    const version = sep2 === -1 ? stem : stem.slice(0, sep2);
...
async function runDoctor(args, deps = {}) {
  ...
  for (const wf of ["pr.yml", "merge.yml"]) {
    try {
      const p = path13.join(projectDir, ".github", "workflows", wf);
  ...
  try {
    switch (args.findingId) {
      case "env-branch-drift": {
        ...
        const envFile = path13.join(args.projectDir, ".env");
        updateEnvConnection({
          envPath: envFile,
...
function resolveSftddDir(projectDir = process.cwd()) {
  const next = join14(projectDir, ARTIFACT_ROOT);
  if (fs13.existsSync(next)) return next;
  const legacy = join14(projectDir, LEGACY_ARTIFACT_ROOT);
  if (fs13.existsSync(legacy)) return legacy;
  return next;
}
var featuresDir = (tdd) => join14(tdd, "features");
var featureDir = (tdd, featureId) => join14(featuresDir(tdd), featureId);
var featureResolved = (tdd, f) => findFeatureDir(tdd, f) ?? featureDir(tdd, f);
var pipelineJson = (tdd, f) => join14(featureResolved(tdd, f), "pipeline.json");
function findFeatureDir(tdd, featureId) {
  const root = featuresDir(tdd);
  if (!fs13.existsSync(root)) return void 0;
  const exact = join14(root, featureId);
  if (fs13.existsSync(exact)) return exact;
  const matches = fs13.readdirSync(root).filter((d) => d === featureId || d.startsWith(`${featureId}-`));
  return matches.length === 1 ? join14(root, matches[0]) : void 0;
}

// scripts/sftdd/stale-branches.ts
init_esm_shims();
import { existsSync as existsSync17, readdirSync as readdirSync10, statSync as statSync5 } from "fs";
import { join as join18 } from "path";

// scripts/sftdd/story-pipeline.ts
init_esm_shims();
import { existsSync as existsSync15, readFileSync as readFileSync9, writeFileSync as writeFileSync6, mkdirSync as mkdirSync5, readdirSync as readdirSync8, statSync as statSync3, rmSync } from "fs";

// scripts/sftdd/gate-conformance-guard.ts
init_esm_shims();
import { existsSync as existsSync14, readFileSync as readFileSync8, readdirSync as readdirSync7, statSync as statSync2 } from "fs";
...
var SCHEMA_DIR = join15(__dirname, "schemas");
...
init_esm_shims();
import { existsSync as existsSync16, mkdirSync as mkdirSync6, readdirSync as readdirSync9, readFileSync as readFileSync10, statSync as statSync4, writeFileSync as writeFileSync7 } from "fs";
import { join as join17 } from "path";
function listSpikes(sftddDir) {
  const root = join17(sftddDir, "spikes");
  if (!existsSync16(root)) return [];
  const out = [];
  for (const slug of readdirSync9(root)) {
    const dir = join17(root, slug);
    if (!statSync4(dir).isDirectory()) continue;
    const branchFile = join17(dir, "branch.txt");
    if (!existsSync16(branchFile)) continue;
    out.push({
      spike_slug: slug,
      branch_id: readFileSync10(branchFile, "utf8").trim(),
      created_at: statSync4(branchFile).birthtime.toISOString(),
      dir
    });
  }
  return out;
...
function listPipelineFeatures(sftddDir) {
  ...
  if (!existsSync17(featuresDir2)) return [];
  return readdirSync10(featuresDir2).filter((d) => statSync5(join18(featuresDir2, d)).isDirectory()).filter((d) => existsSync17(join18(featuresDir2, d, "pipeline.json"))).sort();
}
...
function readEnvProjectId(projectDir) {
  ...
  const lines = fs14.readFileSync(envPath, "utf8").split("\n");
  for (const line of lines) {
