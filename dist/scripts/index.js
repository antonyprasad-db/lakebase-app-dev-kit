...
function findTemplatesDir3() {
  ...
  const here = path7.dirname(fileURLToPath4(import.meta.url));
  let dir = here;
  for (let i = 0; i < 6; i++) {
...
function clientTemplateDir(opts) {
  return path7.join(templatesRoot(opts), "client");
}
function listFilesRelative(dir, relPrefix = "") {
  const out = [];
  for (const entry of fs9.readdirSync(dir)) {
    const p = path7.join(dir, entry);
    const rel = relPrefix ? path7.join(relPrefix, entry) : entry;
...
function deployClientProject(targetDir, projectName, opts) {
  ...
  const destDir = path7.join(targetDir, "client");
  copyDirSubstituted(src, destDir, { projectName });
  return listFilesRelative(destDir).map((r) => path7.join("client", r));
}
...
function adoptTdd(args) {
  ...
  if (!fs10.existsSync(path8.join(args.projectDir, ".git"))) {
    throw new Error(
      `Not a git repo root: ${args.projectDir}. Run \`git init\` first, or pass a path that already has \`.git/\`.`
    );
  }
  const dest = path8.join(args.projectDir, ARTIFACT_ROOT);
  ...
  for (const rel of entries) {
    const fromPath = path8.join(src, rel);
    const toPath = path8.join(dest, rel);
    if (!fs10.existsSync(toPath)) {
      if (!args.dryRun) {
        fs10.mkdirSync(path8.dirname(toPath), { recursive: true });
...
function walkTemplateTree(root) {
  ...
  while (stack.length) {
    ...
    for (const entry of fs10.readdirSync(abs)) {
      const childRel = rel ? path8.join(rel, entry) : entry;
      const childAbs = path8.join(abs, entry);
...
function findBootstrapDir() {
  ...
  const here = path8.dirname(fileURLToPath5(import.meta.url));
  let dir = here;
  for (let i = 0; i < 6; i++) {
    const candidate = path8.join(dir, "templates", "sftdd-bootstrap", ARTIFACT_ROOT);
...
function findTemplatesDir4() {
  ...
  const here = path9.dirname(fileURLToPath6(import.meta.url));
  let dir = here;
  for (let i = 0; i < 6; i++) {
...
async function adoptLakebaseProject(args) {
  ...
  if (!fs14.existsSync(path12.join(args.projectDir, ".git"))) {
    throw new Error(
  ...
  if (args.enableSftdd) {
    if (!dryRun) {
      ...
      for (const rel of result.added) {
        filesWritten.push(path12.join(ARTIFACT_ROOT, rel));
      }
    } else {
...
function _testMakeBrownfieldFixture(opts) {
  ...
  if (opts.packageJson) {
    fs14.writeFileSync(
      path12.join(opts.dir, "package.json"),
...
async function uploadDirectory(args) {
  ...
  const walk = async (dirAbs, dirRel) => {
    for (const entry of readdirSync7(dirAbs)) {
      const childAbs = join15(dirAbs, entry);
      const childRel = dirRel ? `${dirRel}${sep}${entry}` : entry;
      const stat = statSync6(childAbs);
      if (stat.isDirectory()) {
...
import { existsSync as existsSync14, readFileSync as readFileSync12, writeFileSync as writeFileSync10 } from "fs";
...
function readTargets(workspaceRoot) {
  const targetsFile = join16(workspaceRoot, TARGETS_FILE);
  if (!existsSync14(targetsFile)) return null;
  return parseTargetsYaml(readFileSync12(targetsFile, "utf-8"));
}
...
`;
  ...
  writeFileSync10(targetsFile, yaml);
}
...
function buildSchemaQuery(schema) {
  ...
  const join38 = "FROM information_schema.columns c JOIN pg_tables t ON c.table_name = t.tablename AND c.table_schema = t.schemaname ";
  if (isAllSchemas(schema)) {
    return {
      text: `SELECT ${cols} ` + join38 + `WHERE ${SYSTEM_SCHEMA_FILTER} ORDER BY c.table_schema, c.table_name, c.ordinal_position`,
      values: []
    };
  }
  const one = (schema ?? "").trim() || "public";
  return {
    text: `SELECT ${cols} ` + join38 + "WHERE c.table_schema = $1 ORDER BY c.table_name, c.ordinal_position",
    values: [one]
  };
}
...
function verifyHooks(projectDir) {
  ...
  return {
    postCheckout: fs16.existsSync(path14.join(hooksDir, "post-checkout")),
    prepareCommitMsg: fs16.existsSync(path14.join(hooksDir, "prepare-commit-msg")),
    prePush: fs16.existsSync(path14.join(hooksDir, "pre-push"))
  };
}
function verifyWorkflows(projectDir) {
  const wfDir = path14.join(projectDir, ".github", "workflows");
  return {
    pr: fs16.existsSync(path14.join(wfDir, "pr.yml")),
    merge: fs16.existsSync(path14.join(wfDir, "merge.yml"))
  };
}
...
function warmAndVerifyKit(projectDir, timeoutMs = 18e4) {
  const lk = path15.join(projectDir, "scripts", "lk");
...
function cacheDir() {
  return path16.join(os.homedir(), ".cache", "github-actions-runner");
}
function runnersDir() {
  return path16.join(os.homedir(), ".lakebase", "runners");
}
function runnerDir(projectName) {
  return path16.join(runnersDir(), projectName);
}
...
async function ensureCachedArchive() {
  ...
  const cachedPath = path16.join(dir, RUNNER_ARCHIVE);
...
function isRunning(projectName) {
  const pidFile = path16.join(runnerDir(projectName), ".pid");
...
function getRunnerInfo(projectName) {
  ...
  const pidFile = path16.join(dir, ".pid");
  let pid;
...
function stopRunner(projectName) {
  ...
  for (const stale of ["_diag/pages", "_work/_temp", "_work/_actions"]) {
    const full = path16.join(dir, stale);
  ...
  try {
    fs18.mkdirSync(path16.join(dir, "_diag", "pages"), { recursive: true });
  } catch {
  }
}
function resetRunnerConfig(dir, projectName) {
  ...
  for (const f of stateFiles) {
    try {
      fs18.unlinkSync(path16.join(dir, f));
    } catch {
    }
  }
  if (process.platform === "darwin") {
    const plist = path16.join(
      os.homedir(),
      "Library",
      "LaunchAgents",
...
async function setupRunner(args) {
  ...
  if (child.pid) {
    fs18.writeFileSync(path16.join(dir, ".pid"), String(child.pid));
  }
...
import { existsSync as existsSync19, readFileSync as readFileSync15, mkdirSync as mkdirSync11, writeFileSync as writeFileSync13 } from "fs";
...
var AGENT_CONFIG_REL = join21(".lakebase", "agent-config.json");

// scripts/sftdd/sftdd-config.ts
var SFTDD_CONFIG_REL = join22(".lakebase", "sftdd-config.json");
var LEGACY_TDD_CONFIG_REL = join22(".lakebase", "tdd-config.json");
...
function writeSftddConfig(projectDir, config, opts) {
  const f = join22(projectDir, TDD_CONFIG_REL);
  if (existsSync19(f) && !opts?.force) return false;
  mkdirSync11(dirname8(f), { recursive: true });
  writeFileSync13(f, JSON.stringify(config, null, 2) + "\n");
  return true;
}
      ...
      if (enableSftdd) {
        ...
        if (kitRef) {
          try {
            ...
            fs20.writeFileSync(path18.join(dir, "kit-ref"), `${kitRef}
`, "utf8");
          } catch (err) {
...
function layDownTddScaffold(targetDir) {
  const candidates = [
    path18.resolve(__dirname, `../../templates/sftdd-bootstrap/${ARTIFACT_ROOT}`),
    path18.resolve(__dirname, `../../../templates/sftdd-bootstrap/${ARTIFACT_ROOT}`)
  ];
  ...
  const dest = path18.join(targetDir, ARTIFACT_ROOT);
...
function readKitVersion(kitCommandsDir) {
  ...
  try {
    const raw = fs31.readFileSync(path30.join(dir, "package.json"), "utf-8");
    const pkg = JSON.parse(raw);
    return typeof pkg.version === "string" ? pkg.version : "unknown";
  } catch {
    return "unknown";
...
function detectWorkflowDrift(args) {
  const projectWorkflowsDir = path31.join(
    args.projectDir,
    ".github",
    "workflows"
  );
  const here = path31.dirname(new URL(import.meta.url).pathname);
  const kitWorkflowsDir = args.kitDir ? path31.join(
    args.kitDir,
    "templates",
    "project",
    "common",
  ...
  for (const name of templateFiles) {
    seen.add(name);
    const projectPath2 = path31.join(projectWorkflowsDir, name);
    const templatePath = path31.join(kitWorkflowsDir, name);
...
function readKitVersion2(kitWorkflowsDir) {
  ...
  try {
    const raw = fs32.readFileSync(path31.join(dir, "package.json"), "utf-8");
    const pkg = JSON.parse(raw);
    return typeof pkg.version === "string" ? pkg.version : "unknown";
  } catch {
    return "unknown";
...
function detectCommandDrift(args) {
  const projectCommandsDir = path31.join(args.projectDir, ".claude", "commands");
  const here = path31.dirname(new URL(import.meta.url).pathname);
  const kitCommandsDir = args.kitDir ? path31.join(args.kitDir, "templates", "project", "common", ".claude", "commands") : findKitCommandsDir2(here);
  ...
  for (const name of templateFiles) {
    seen.add(name);
    const projectPath2 = path31.join(projectCommandsDir, name);
    const templatePath = path31.join(kitCommandsDir, name);
...
async function commitAllIfChanged(args) {
  ...
  for (const inc of args.include ?? []) {
    if (existsSync32(join36(args.cwd, inc))) {
