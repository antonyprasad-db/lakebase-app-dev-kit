...
__export(optimize_cli_exports, {
  actionToHandoffPlan: () => actionToHandoffPlan,
  isBuildHandoff: () => isBuildHandoff,
...
function experimentsRoot(sftddDir, featureId, storyId) {
  return (0, import_path7.join)(sftddDir, "experiments", featureId, storyId);
}
function experimentDir(sftddDir, featureId, storyId, slug) {
  return (0, import_path7.join)(experimentsRoot(sftddDir, featureId, storyId), slug);
}
async function cutExperiment(args, deps = {}) {
  ...
  (0, import_fs7.writeFileSync)((0, import_path7.join)(dir, "branch.txt"), branchId);
  (0, import_fs7.writeFileSync)(
    (0, import_path7.join)(dir, "notes.md"),
...
`
  ...
  (0, import_fs7.writeFileSync)((0, import_path7.join)(dir, "outcomes.json"), JSON.stringify(outcomes, null, 2) + "\n");
  (0, import_fs7.writeFileSync)(
    (0, import_path7.join)(dir, "timeline.json"),
    JSON.stringify(
...
function acIdsInStoryDir(storyDir2) {
  const dir = (0, import_path8.join)(storyDir2, "acs");
  ...
  for (const f of (0, import_fs8.readdirSync)(dir)) {
    ...
    try {
      const obj = JSON.parse((0, import_fs8.readFileSync)((0, import_path8.join)(dir, f), "utf8"));
...
function readSmellsLog(sftddDir) {
  ...
  if (!(0, import_fs9.existsSync)(file)) return { detected: [] };
  return JSON.parse((0, import_fs9.readFileSync)(file, "utf8"));
}
...
function storyTestListFingerprint(sftddDir, featureId, story_id) {
  ...
  try {
    return (0, import_crypto.createHash)("sha1").update((0, import_fs9.readFileSync)(f)).digest("hex");
  } catch {
    return "";
  }
}
...
function storyCycles(sftddDir, featureId, story) {
  const base = (0, import_path10.join)(cyclesRootDir(sftddDir), featureId, story);
...
function gatesFilePath(sftddDir, featureId) {
  return (0, import_path11.join)(requireFeatureDir(sftddDir, featureId), "gates.json");
}
...
function projectBriefRefs(sftddDir) {
  ...
  if (!(0, import_fs14.existsSync)(fdir)) return refs;
  for (const feature of (0, import_fs14.readdirSync)(fdir)) {
    const archPath = (0, import_path12.join)(fdir, feature, "architecture.json");
    if (!(0, import_fs14.existsSync)(archPath)) continue;
    try {
      const parsed = JSON.parse((0, import_fs14.readFileSync)(archPath, "utf8"));
...
function canonicalArtifactName(path9) {
  const base = (0, import_path12.basename)(path9);
  if ((0, import_path12.basename)((0, import_path12.dirname)(path9)) === "acs" && base.endsWith(".json")) return "ac.json";
  return base;
}
...
function storyCycles2(sftddDir, featureId, story) {
  ...
  for (const acDir of fs12.readdirSync(base)) {
    ...
    for (const f of fs12.readdirSync(dir)) {
      if (!/^cycle-\d+\.json$/.test(f)) continue;
      try {
        out.push(JSON.parse(fs12.readFileSync(path8.join(dir, f), "utf8")));
      } catch {
      }
    }
  }
...
function storyAcProblems(fdir, story) {
  const acsDir2 = (0, import_node_path9.join)(fdir, "stories", story, "acs");
  if (!(0, import_node_fs7.existsSync)(acsDir2)) return [];
  const problems = [];
  const acs = [];
  for (const f of (0, import_node_fs7.readdirSync)(acsDir2)) {
    if (!f.endsWith(".json")) continue;
    const p = (0, import_node_path9.join)(acsDir2, f);
    let content;
    try {
      content = (0, import_node_fs7.readFileSync)(p, "utf8");
    } catch {
      continue;
    }
...
function acsConformanceReason(fdir) {
  const stories = (0, import_node_path9.join)(fdir, "stories");
  if (!(0, import_node_fs7.existsSync)(stories)) return null;
  const problems = (0, import_node_fs7.readdirSync)(stories).flatMap((s) => storyAcProblems(fdir, s));
  return problems.length === 0 ? null : `AC conformance failed: ${problems.join("; ")}`;
}
function collectStoryJsons(fdir) {
  const stories = (0, import_node_path9.join)(fdir, "stories");
  if (!(0, import_node_fs7.existsSync)(stories)) return [];
  const out = [];
  for (const s of (0, import_node_fs7.readdirSync)(stories)) {
    const p = (0, import_node_path9.join)(stories, s, "story.json");
    if (!(0, import_node_fs7.existsSync)(p)) continue;
    try {
      out.push({ name: s, content: (0, import_node_fs7.readFileSync)(p, "utf8") });
    } catch {
      continue;
    }
  }
...
function architectureConventionsReason(sftddDir, featureId) {
  ...
  const archFile = architectureJson(sftddDir, featureId);
  if (!(0, import_node_fs7.existsSync)(archFile)) return null;
  let content;
  try {
    content = (0, import_node_fs7.readFileSync)(archFile, "utf8");
  } catch {
    return null;
  }
...
function readArchitecture(sftddDir, featureId) {
  ...
  if (!(0, import_node_fs7.existsSync)(f)) return void 0;
  try {
...
function dbDesignReason(sftddDir, featureId) {
  ...
  if (arch === void 0) return null;
  const dbFile = dbDesignJson(sftddDir, featureId);
  const db = (0, import_node_fs7.existsSync)(dbFile) ? (() => {
    try {
      return (0, import_node_fs7.readFileSync)(dbFile, "utf8");
    } catch {
      return void 0;
    }
  })() : void 0;
...
function nfrCoverageReason(sftddDir, featureId) {
  ...
  const nfrsFile = (0, import_node_fs7.existsSync)(featureNfrs) ? featureNfrs : (0, import_node_fs7.existsSync)(projectNfrs) ? projectNfrs : void 0;
  if (nfrsFile === void 0) return null;
  let nfrsContent;
  try {
    nfrsContent = (0, import_node_fs7.readFileSync)(nfrsFile, "utf8");
  } catch {
    return null;
  }
...
function invariantCoverageDistinctReason(sftddDir, featureId, testListJson) {
  ...
  const storiesDir2 = (0, import_node_path9.join)(featureDir2(sftddDir, featureId), "stories");
  if (!(0, import_node_fs7.existsSync)(storiesDir2)) return null;
  const perStory = (0, import_node_fs7.readdirSync)(storiesDir2).filter((s) => {
    try {
      return (0, import_node_fs7.statSync)((0, import_node_path9.join)(storiesDir2, s)).isDirectory();
    } catch {
      return false;
    }
  }).map((story) => {
...
function serviceBackedReason(sftddDir, featureId) {
  ...
  const stories = (0, import_node_path9.join)(fdir, "stories");
  if ((0, import_node_fs7.existsSync)(stories)) {
    for (const s of (0, import_node_fs7.readdirSync)(stories)) {
      const ad = (0, import_node_path9.join)(stories, s, "acs");
      if (!(0, import_node_fs7.existsSync)(ad)) continue;
      for (const f of (0, import_node_fs7.readdirSync)(ad)) {
        if (!f.endsWith(".json")) continue;
        try {
          const layer = JSON.parse((0, import_node_fs7.readFileSync)((0, import_node_path9.join)(ad, f), "utf8")).layer;
...
function resolveArtifactInputs(gate, fdir, promoteRef, sftddDir, featureId) {
  const readIfPresent = (name) => {
    const p = (0, import_node_path9.join)(fdir, name);
    try {
      return (0, import_node_fs7.existsSync)(p) ? (0, import_node_fs7.readFileSync)(p, "utf8") : void 0;
    } catch {
      return void 0;
    }
  };
...
function checkSpecAuthorBreakdown(sftddDir, featureId, v) {
  ...
  if (!(0, import_node_fs8.existsSync)(specPath)) {
  ...
  try {
    const spec = JSON.parse((0, import_node_fs8.readFileSync)(specPath, "utf8"));
  ...
  if (!(0, import_node_fs8.existsSync)(sdir)) return;
  const storyJsons = [];
  for (const s of (0, import_node_fs8.readdirSync)(sdir)) {
    const p = `${sdir}/${s}/story.json`;
    if (!(0, import_node_fs8.existsSync)(p)) continue;
    try {
      storyJsons.push({ name: s, content: (0, import_node_fs8.readFileSync)(p, "utf8") });
    } catch {
      continue;
    }
  }
...
function checkSpecAuthor(args, v) {
  ...
  for (const f of (0, import_node_fs8.readdirSync)(dir)) {
    ...
    let content;
    try {
      content = (0, import_node_fs8.readFileSync)(`${dir}/${f}`, "utf8");
    } catch {
      continue;
    }
...
function checkNfrFitnessFunctions(sftddDir, featureId, v) {
  const archFile = architectureJson(sftddDir, featureId);
  if (!(0, import_node_fs8.existsSync)(archFile)) return;
  let nfrs;
  try {
    nfrs = JSON.parse((0, import_node_fs8.readFileSync)(archFile, "utf8")).nfrs ?? [];
  } catch {
    return;
  }
...
function checkDba(args, v) {
  ...
  const archFile = architectureJson(sftddDir, featureId);
  if (!(0, import_node_fs8.existsSync)(archFile)) {
    v.push({ artifact: "architecture.json", problem: "architecture.json missing (the architect owns the contract the DBA realizes)" });
    return;
  }
  const archContent = (0, import_node_fs8.readFileSync)(archFile, "utf8");
  const dbFile = dbDesignJson(sftddDir, featureId);
  const dbContent = (0, import_node_fs8.existsSync)(dbFile) ? (0, import_node_fs8.readFileSync)(dbFile, "utf8") : void 0;
  if (dbContent !== void 0) {
...
function designGuideHasComponents(sftddDir) {
  const file = designGuideJson(sftddDir);
  if (!(0, import_node_fs8.existsSync)(file)) return { ok: true };
  try {
...
function buildDriveEffects(cfg) {
  return {
    ...
    onHandback(handoff, detail) {
      ...
      try {
        fs13.mkdirSync((0, import_node_path10.dirname)(file), { recursive: true });
...
function resolveStepReference(args) {
  ...
  const root = (0, import_node_path11.join)(kitRoot2, SCENARIOS_REL, corpus, "recorded-artifacts");
  if (!(0, import_node_fs9.existsSync)(root)) return null;
  if (step === "acs") {
    const sdir = storiesDir(root, featureId);
    if (!(0, import_node_fs9.existsSync)(sdir)) return null;
    const paths = [];
    for (const story of (0, import_node_fs9.readdirSync)(sdir)) {
      const adir = acsDir(root, featureId, story);
      if (!(0, import_node_fs9.existsSync)(adir)) continue;
      for (const ac of (0, import_node_fs9.readdirSync)(adir)) if (ac.endsWith(".json")) paths.push((0, import_node_path11.join)(adir, ac));
    }
  ...
  if (!p || !(0, import_node_fs9.existsSync)(p)) return null;
  return { corpus, paths: [p], label: p.slice(root.length + 1) };
}
function readCandidateArtifact(args) {
  const { sftddDir, step, featureId } = args;
  const readIf = (p2) => (0, import_node_fs9.existsSync)(p2) ? (0, import_node_fs9.readFileSync)(p2, "utf8") : null;
  if (step === "acs") {
    const sdir = storiesDir(sftddDir, featureId);
    if (!(0, import_node_fs9.existsSync)(sdir)) return null;
    const parts = [];
    for (const story of (0, import_node_fs9.readdirSync)(sdir)) {
      ...
      if (!(0, import_node_fs9.existsSync)(adir)) continue;
      for (const ac of (0, import_node_fs9.readdirSync)(adir)) if (ac.endsWith(".json")) parts.push((0, import_node_fs9.readFileSync)((0, import_node_path11.join)(adir, ac), "utf8"));
    }
...
async function evaluateSemanticGate(args) {
  ...
  const reference = ref.paths.map((p) => (0, import_node_fs9.readFileSync)(p, "utf8")).join("\n---\n");
...
function overlayAgent(args) {
  ...
  const agentPath = (0, import_node_path12.join)(projectDir, ".claude", "agents", `${role}.md`);
  const hadBaseline = (0, import_node_fs10.existsSync)(agentPath);
  const baseline = hadBaseline ? (0, import_node_fs10.readFileSync)(agentPath, "utf8") : void 0;
  (0, import_node_fs10.mkdirSync)((0, import_node_path12.dirname)(agentPath), { recursive: true });
  (0, import_node_fs10.writeFileSync)(agentPath, markdown);
  return {
    restore() {
      if (hadBaseline) {
        (0, import_node_fs10.writeFileSync)(agentPath, baseline);
      } else if ((0, import_node_fs10.existsSync)(agentPath)) {
        (0, import_node_fs10.rmSync)(agentPath, { force: true });
      }
    }
  };
}
...
function snapshotDesign(args) {
  ...
  const backupTree = (0, import_node_path13.join)(backup, (0, import_node_path13.basename)(sftddDir));
  (0, import_node_fs11.cpSync)(sftddDir, backupTree, { recursive: true });
  return {
    restore() {
      (0, import_node_fs11.rmSync)(sftddDir, { recursive: true, force: true });
      (0, import_node_fs11.cpSync)(backupTree, sftddDir, { recursive: true });
    },
    dispose() {
...
function sha1(abs) {
  return (0, import_node_crypto3.createHash)("sha1").update((0, import_node_fs12.readFileSync)(abs)).digest("hex");
}
...
function walk(dir, keep) {
  ...
  for (const entry of (0, import_node_fs12.readdirSync)(dir)) {
    const abs = (0, import_node_path14.join)(dir, entry);
    if (keep && !keep(abs)) continue;
    let st;
    try {
      st = (0, import_node_fs12.statSync)(abs);
    } catch {
      continue;
    }
...
function scan(projectDir, sftddDir) {
  ...
  for (const abs of walk(sftddDir)) {
    const rel = (0, import_node_path14.relative)(projectDir, abs);
    if (NON_ARTIFACT_TDD.has((0, import_node_path14.relative)(sftddDir, abs))) continue;
...
function writeRecorderState(recordDir, cur) {
  ...
  (0, import_node_fs12.mkdirSync)(recordDir, { recursive: true });
  (0, import_node_fs12.writeFileSync)((0, import_node_path14.join)(recordDir, ".recorder-state.json"), JSON.stringify({ files }, null, 2) + "\n");
}
function seedRecorderBaseline(args) {
  if ((0, import_node_fs12.existsSync)((0, import_node_path14.join)(args.recordDir, ".recorder-state.json"))) return false;
  writeRecorderState(args.recordDir, scan(args.projectDir, args.sftddDir));
  return true;
}
function readState(recordDir) {
  const f = (0, import_node_path14.join)(recordDir, ".recorder-state.json");
  if (!(0, import_node_fs12.existsSync)(f)) return { files: {} };
  try {
    return JSON.parse((0, import_node_fs12.readFileSync)(f, "utf8"));
  } catch {
    return { files: {} };
  }
}
function readIndex(recordDir) {
  const f = (0, import_node_path14.join)(recordDir, "turns", "index.json");
  if (!(0, import_node_fs12.existsSync)(f)) return [];
  try {
    const data = JSON.parse((0, import_node_fs12.readFileSync)(f, "utf8"));
...
function recordTurn(args) {
  ...
  const turnDir = (0, import_node_path14.join)(recordDir, "turns", dirName);
  (0, import_node_fs12.mkdirSync)((0, import_node_path14.join)(turnDir, "files"), { recursive: true });
  const artifactsDir = (0, import_node_path14.join)(recordDir, "recorded-artifacts");
  for (const rel of produced) {
    const f = cur.get(rel);
    const dst = (0, import_node_path14.join)(turnDir, "files", rel);
    (0, import_node_fs12.mkdirSync)((0, import_node_path14.dirname)(dst), { recursive: true });
    (0, import_node_fs12.cpSync)(f.abs, dst);
    if (f.underTdd) {
      const mirror = (0, import_node_path14.join)(artifactsDir, (0, import_node_path14.relative)(sftddDir, f.abs));
      (0, import_node_fs12.mkdirSync)((0, import_node_path14.dirname)(mirror), { recursive: true });
      (0, import_node_fs12.cpSync)(f.abs, mirror);
    }
  }
  for (const rel of deleted) {
    const abs = (0, import_node_path14.join)(projectDir, rel);
    if (abs.startsWith(sftddDir)) {
      const mirror = (0, import_node_path14.join)(artifactsDir, (0, import_node_path14.relative)(sftddDir, abs));
      if ((0, import_node_fs12.existsSync)(mirror)) (0, import_node_fs12.rmSync)(mirror, { force: true });
    }
  }
  let transcriptSummary;
  if (transcript) {
    (0, import_node_fs12.writeFileSync)((0, import_node_path14.join)(turnDir, "transcript.md"), renderTranscriptMd(transcript, label));
    transcriptSummary = {
  ...
  (0, import_node_fs12.writeFileSync)((0, import_node_path14.join)(turnDir, "turn.json"), JSON.stringify(manifest, null, 2) + "\n");
  ...
  index.push(entry);
  (0, import_node_fs12.mkdirSync)((0, import_node_path14.join)(recordDir, "turns"), { recursive: true });
  (0, import_node_fs12.writeFileSync)((0, import_node_path14.join)(recordDir, "turns", "index.json"), JSON.stringify({ turns: index }, null, 2) + "\n");
...
function writeTrialRecord(ctx, handoff, candidate, trial, result) {
  const dir = (0, import_node_path15.join)(ctx.experimentsDir, handoff.id, candidate.id, `trial-${trial}`);
  (0, import_node_fs13.mkdirSync)(dir, { recursive: true });
  (0, import_node_fs13.writeFileSync)((0, import_node_path15.join)(dir, "candidate.json"), JSON.stringify(candidate, null, 2) + "\n");
  (0, import_node_fs13.writeFileSync)((0, import_node_path15.join)(dir, "result.json"), JSON.stringify(result, null, 2) + "\n");
}
...
`);
      ...
      const champ = (0, import_node_path15.join)(ctx.experimentsDir, "champion-walk.json");
      const prior = (0, import_node_fs13.existsSync)(champ) ? JSON.parse((0, import_node_fs13.readFileSync)(champ, "utf8")) : { winners: [] };
      prior.winners.push({ handoffId: handoff.id, candidateId: candidate.id });
      (0, import_node_fs13.mkdirSync)(ctx.experimentsDir, { recursive: true });
      (0, import_node_fs13.writeFileSync)(champ, JSON.stringify(prior, null, 2) + "\n");
    }
  };
}
...
function buildCtxForHandoff(handoff, loc) {
  ...
  const ctx = {
    ...
    sftddDir,
    featureId,
    experimentsDir: (0, import_node_path16.join)(projectDir, "experiments"),
...
async function main() {
  ...
  const projectDir = (0, import_node_path16.resolve)(args.projectDir ?? process.cwd());
