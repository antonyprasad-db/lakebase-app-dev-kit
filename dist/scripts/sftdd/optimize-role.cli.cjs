...
// consort/orchestrator/manifest/step-manifest.ts
init_cjs_shims();
...
// consort/orchestrator/config/step-manifests/spec-author-breakdown.json
...
// consort/orchestrator/config/step-manifests/spec-author-propose.json
...
// consort/orchestrator/config/step-manifests/spec-author-story.json
...
// consort/orchestrator/config/step-manifests/architect-estimator.json
...
// consort/orchestrator/config/step-manifests/architect-reviewer.json
...
// consort/orchestrator/config/step-manifests/dba.json
var dba_default = {
  id: "dba",
  role: "dba",
...
// consort/orchestrator/config/step-manifests/test-strategist.json
...
// consort/orchestrator/config/step-manifests/driver-green.json
var driver_green_default = {
  id: "driver-green",
  role: "driver",
  match: { kind: "invoke-role", role: "driver", buildMode: null, ac: null },
  inputs: [
    { id: "test-list", source: "story:test-list.json", description: "The story's ordered failing tests the Driver makes GREEN." }
  ],
  outputs: [],
  routing: {
    produced: { next: "state-derived" }
  ...
  postTurn: [
    { bin: "CYCLE_BIN", args: ["green", "--feature", "{feature}", "--story", "{story}", "--tdd-dir", "{tddDir}"], when: "after" }
  ]
};

// consort/orchestrator/config/step-manifests/ux-designer.json
...
// consort/orchestrator/steps/manifest-step.ts
init_cjs_shims();
...
var ManifestStep = class {
  ...
  async run(provided) {
    ...
    for (const spec of this.outputs(action)) {
      ...
      const p = (0, import_node_path3.join)(workspaceDir, rel);
      if (this.exists(p)) {
  ...
  return {
    async invoke(invocation) {
      ...
      for (const seed of opts.seeds) {
        const src = (0, import_node_path5.join)(opts.corpusRoot, seed.from);
        ...
        const dst = (0, import_node_path5.join)(invocation.workspaceDir, seed.to);
        (0, import_node_fs5.mkdirSync)((0, import_node_path5.dirname)(dst), { recursive: true });
      ...
      const logPath = (0, import_node_path5.join)(invocation.workspaceDir, "agent-log.jsonl");
  ...
  return {
    async invoke(invocation) {
      for (const [filename, contents] of Object.entries(outputs)) {
        (0, import_node_fs6.writeFileSync)((0, import_node_path6.join)(invocation.workspaceDir, filename), contents);
      }
      const logPath = (0, import_node_path6.join)(invocation.workspaceDir, "agent-log.jsonl");
...
function storyTestListFingerprint(sftddDir, featureId, story_id) {
  ...
  if (!(0, import_fs2.existsSync)(f)) return "";
  try {
    return (0, import_crypto.createHash)("sha1").update((0, import_fs2.readFileSync)(f)).digest("hex");
  } catch {
    return "";
  }
}
...
function storyDeployVerified(sftddDir, featureId, storyId) {
  ...
  return deployEvidencePasses(readDeployEvidence((0, import_node_path8.join)(fdir, "stories", storyId, "deploy-evidence.json")));
}
...
function supersededTestsJson(tdd, feature, story, ac) {
  return (0, import_node_path10.join)(cycleDir(tdd, feature, story, ac), "superseded-tests.json");
}
...
function greenFailureJson(tdd, feature, story, ac) {
  return (0, import_node_path10.join)(cycleDir(tdd, feature, story, ac), "green-failure.json");
}
...
function storyCycles(sftddDir, featureId, story) {
  ...
  for (const acDir of (0, import_fs3.readdirSync)(base)) {
    ...
    for (const f of (0, import_fs3.readdirSync)(dir)) {
      ...
      try {
        out.push(JSON.parse((0, import_fs3.readFileSync)((0, import_path5.join)(dir, f), "utf8")));
      } catch {
      }
    }
  }
...
function readReview(sftddDir, featureId, story, acId) {
  ...
  if (!(0, import_fs3.existsSync)(f)) return {};
  try {
...
function readCanon(sftddDir) {
  ...
  if (!(0, import_fs5.existsSync)(f)) return void 0;
  try {
...
// consort/orchestrator/state/escalation-probe.ts
function deriveEscalation(sftddDir, featureId, buildActive = null) {
  return diskArtifactProbe(sftddDir, featureId, buildActive).pendingEscalation();
}
function probeDriveState(sftddDir, featureId, buildActive = null) {
  return {
    phase: "feature",
    breakdownDone: true,
    storyOrder: [],
...
// consort/orchestrator/execution/step-executor.ts
init_cjs_shims();
var import_node_fs12 = require("fs");
var import_node_path13 = require("path");

// consort/orchestrator/contract/step-contract.ts
init_cjs_shims();
...
async function execute(step, ctx, deps) {
  ...
  for (const spec of step.outputs(action)) {
    ...
    const abs = producedPaths.find((p) => p.endsWith(rel)) ?? (0, import_node_path13.join)(workspaceDir, rel);
...
// consort/orchestrator/execution/agent-report-formatter.ts
init_cjs_shims();
...
function formatAgentReport(args) {
  const logPath = (0, import_node_path14.join)(args.workspaceDir, args.logFile ?? "agent-log.jsonl");
  let reportJson;
  let source;
  if (args.reportText !== void 0) {
    ...
    const reportPath = (0, import_node_path14.join)(args.workspaceDir, args.reportFile ?? ".agent-report.json");
    source = reportPath;
  ...
  if ((0, import_node_fs13.existsSync)(logPath)) {
    ...
    (0, import_node_fs13.mkdirSync)((0, import_node_path14.dirname)(logPath), { recursive: true });
...
function resolveInputsFromWorkspace(manifest, workspaceDir) {
  ...
  for (const input of manifest.inputs) {
    ...
    const p = (0, import_node_path15.join)(workspaceDir, file);
    if (!(0, import_node_fs14.existsSync)(p)) return { missing: input.id };
...
function executorWiring(manifest, action, deps, retries) {
  ...
  const state = deps.state ?? (deps.probeEscalation ? probeDriveState(deps.cfg.sftddDir, deps.cfg.featureId) : { phase: "feature" });
  const ctx = {
    action,
    cfg: deps.cfg,
    state,
...
function layDownKitAgents(workspaceDir, kitDir = process.cwd()) {
  const src = (0, import_node_path16.join)(kitDir, "skills", "consort", "agents");
  if (!(0, import_node_fs15.existsSync)(src)) throw new Error(`layDownKitAgents: kit agents dir not found at ${src}`);
  const dest = (0, import_node_path16.join)(workspaceDir, ".claude", "agents");
...
async function runIntegrationChain(config) {
  ...
  const workspaceDir = (0, import_node_fs15.mkdtempSync)((0, import_node_path16.join)((0, import_node_os.tmpdir)(), "integration-chain-"));
  (0, import_node_fs15.mkdirSync)((0, import_node_path16.join)(workspaceDir, ".sftdd"), { recursive: true });
  ...
  const runnerDeps = {
    ...
    cfg: {
      ...
      sftddDir: (0, import_node_path16.join)(workspaceDir, ".sftdd"),
      featureId: config.feature
    },
    agentContext,
    formatAgentReports: true,
    ...config.instructionsFor ? { instructionsFor: (m) => config.instructionsFor(m) } : {},
  ...
  try {
    ...
    const producedArtifacts = snapshotTree((0, import_node_path16.join)(workspaceDir, ".sftdd"), workspaceDir);
    return { turns, workspaceDir, producedArtifacts };
  } finally {
    (0, import_node_fs15.rmSync)(workspaceDir, { recursive: true, force: true });
  }
}
function snapshotTree(root, relTo) {
  const out = {};
  if (!(0, import_node_fs15.existsSync)(root)) return out;
  const walk = (dir) => {
    for (const entry of (0, import_node_fs15.readdirSync)(dir, { withFileTypes: true })) {
      const abs = (0, import_node_path16.join)(dir, entry.name);
      if (entry.isDirectory()) walk(abs);
      else if (entry.isFile()) out[(0, import_node_path16.relative)(relTo, abs)] = (0, import_node_fs15.readFileSync)(abs, "utf8");
    }
  };
  walk(root);
  return out;
...
Declare feature_id, tables[] (columns with type/nullable, primary_key, unique_constraints, foreign_keys, checks, indexes), this story's schema_changes[], and realizes_invariants[] as a FLAT array of the architecture.json persistence_invariant id STRINGS (bare ids, not objects). Do NOT re-author the invariants; physically realize them.` + NO_SHELL + REPORT_BLOCK
  ...
  "test-strategist": {
    ...
    prompt: `You are the Test Strategist. From the provided inputs (the story AC + architecture.json + db-design.json, in this prompt), produce the feature master test list. WRITE exactly this file, relative to your current working directory:
  - features/${FEATURE}/test-list.json
Order the story's tests; map each test's ac_id to the provided AC's exact id. Cover EVERY architecture.json persistence_invariant with a real-branch fitness test that sets "invariant_id". Every DB-writing test must own its state (a per-run-unique key). Conform to test-list.schema.json.` + NO_SHELL + REPORT_BLOCK
  },
  "spec-author-propose": {
...
async function runRoleChainLive(chain, opts = {}) {
  ...
  const { turns, producedArtifacts } = await runIntegrationChain({
    manifestDir: (0, import_node_path17.join)(kit, MANIFESTS_REL, chain.dir),
    intakeDir: (0, import_node_path17.join)(kit, INTAKE_REL),
    feature: FEATURE,
    start: PO_SEED,
...
function roleCandidates(baseModel) {
...
function reportRoleSweep(trials) {
...
// scripts/sftdd/optimize-role.cli.ts
function parseArgs(argv, chains = ROLE_CHAINS) {
  const get = (flag) => {
...
async function runOptimizeRole(args) {
  ...
  const runDir = args.telemetryDir ?? (0, import_node_path20.join)(process.cwd(), ".role-telemetry", `sweep-${args.role}-${runStamp()}`);
  ...
  (0, import_node_fs18.writeFileSync)((0, import_node_path20.join)(runDir, "report.txt"), formatRoleSweepReport(report) + "\n");
...
function readReference(chain, _role) {
  const p = (0, import_node_path20.join)(process.cwd(), INTAKE_REL, chain.outputFile);
...
function persistTrial(runDir, chain, baseModel, trial) {
  const dir = (0, import_node_path20.join)(runDir, trial.candidateId);
  (0, import_node_fs18.mkdirSync)(dir, { recursive: true });
  if (trial.telemetry) (0, import_node_fs18.writeFileSync)((0, import_node_path20.join)(dir, "telemetry.json"), JSON.stringify(trial.telemetry, null, 2) + "\n");
  for (const [rel, contents] of Object.entries(trial.producedArtifacts ?? {})) {
    const dest = (0, import_node_path20.join)(dir, "artifacts", rel);
    (0, import_node_fs18.mkdirSync)((0, import_node_path20.dirname)(dest), { recursive: true });
    (0, import_node_fs18.writeFileSync)(dest, contents);
  }
  const replay = {
    role: chain.dir,
  ...
  (0, import_node_fs18.writeFileSync)((0, import_node_path20.join)(dir, "replay.json"), JSON.stringify(replay, null, 2) + "\n");
}
