...
async function commitAllIfChanged(args) {
  ...
  for (const inc of args.include ?? []) {
    if ((0, import_fs5.existsSync)((0, import_path5.join)(args.cwd, inc))) {
...
var AlembicAdapter = {
   ...
   */
  detect(projectDir) {
    if (fs9.existsSync(path5.join(projectDir, "alembic.ini"))) return true;
    if (fs9.existsSync(path5.join(projectDir, "migrations", "env.py"))) return true;
    if (fs9.existsSync(path5.join(projectDir, "alembic", "env.py"))) return true;
    return false;
  },
  async apply(args) {
...
var FlywayAdapter = {
  ...
  detect(projectDir) {
    return fs10.existsSync(path7.join(projectDir, "pom.xml"));
  },
  async apply(args) {
...
var KnexAdapter = {
   ...
   */
  detect(projectDir) {
    return KNEXFILE_VARIANTS2.some((name) => fs12.existsSync(path9.join(projectDir, name)));
  },
  async apply(args) {
