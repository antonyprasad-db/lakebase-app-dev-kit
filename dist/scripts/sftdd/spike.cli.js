...
async function cutSpike(args) {
  ...
  const dir = join5(sftddDir, "spikes", spikeSlug);
  mkdirSync3(dir, { recursive: true });
  writeFileSync4(join5(dir, "branch.txt"), branchId);
  writeFileSync4(
    join5(dir, "notes.md"),
    notes ?? `# ${spikeSlug}
...
function listSpikes(sftddDir) {
  const root = join5(sftddDir, "spikes");
...
async function deleteSpike(args) {
  ...
  const dir = join5(sftddDir, "spikes", spikeSlug);
  if (!existsSync5(dir)) throw new Error(`spike ${spikeSlug} not found at ${dir}`);
  if (deleteBranchToo) {
    const branchId = readFileSync5(join5(dir, "branch.txt"), "utf8").trim();
