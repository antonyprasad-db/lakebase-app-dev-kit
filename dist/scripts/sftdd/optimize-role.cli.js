...
var ManifestStep = class {
  constructor(manifest, agent, exists) {
    ...
    this.agent = agent;
    this.exists = exists ?? existsSync3;
  }
  manifest;
  agent;
  exists;
  ...
  async run(provided) {
    ...
    for (const spec of this.outputs(action)) {
      ...
      const p = join4(workspaceDir, rel);
      if (this.exists(p)) {
...
// scripts/sftdd/sftdd-config.ts
var SFTDD_CONFIG_REL = join6(".lakebase", "sftdd-config.json");
var LEGACY_TDD_CONFIG_REL = join6(".lakebase", "tdd-config.json");
...
init_esm_shims();
import { existsSync as existsSync9, mkdirSync as mkdirSync6, cpSync as cpSync2, rmSync, readdirSync as readdirSync6, statSync as statSync3 } from "fs";
import { join as join11, dirname as dirname7, basename } from "path";
...
init_esm_shims();
import { readFileSync as readFileSync10, writeFileSync as writeFileSync5, existsSync as existsSync10, mkdirSync as mkdirSync7 } from "fs";
  ...
  return {
    async invoke(invocation) {
      ...
      for (const seed of opts.seeds) {
        const src = join12(opts.corpusRoot, seed.from);
        if (!existsSync10(src)) {
          throw new Error(
        ...
        const dst = join12(invocation.workspaceDir, seed.to);
        mkdirSync7(dirname8(dst), { recursive: true });
        writeFileSync5(dst, readFileSync10(src, "utf8"));
      ...
      const logPath = join12(invocation.workspaceDir, "agent-log.jsonl");
      const prior = existsSync10(logPath) ? readFileSync10(logPath, "utf8") : "";
      writeFileSync5(logPath, prior + JSON.stringify(event) + "\n");
    }
  };
}
  ...
  return {
    async invoke(invocation) {
      for (const [filename, contents] of Object.entries(outputs)) {
        writeFileSync6(join13(invocation.workspaceDir, filename), contents);
      }
      const logPath = join13(invocation.workspaceDir, "agent-log.jsonl");
      const prior = existsSync11(logPath) ? readFileSync11(logPath, "utf8") : "";
      ...
      writeFileSync6(logPath, prior + JSON.stringify(event) + "\n");
    }
  };
}
var AGENT_CATALOGUE = {
...
init_esm_shims();
import { existsSync as existsSync21, readFileSync as readFileSync22, readdirSync as readdirSync14, statSync as statSync8, writeFileSync as writeFileSync13, mkdirSync as mkdirSync13, rmSync as rmSync6 } from "fs";
...
import { existsSync as existsSync15, mkdirSync as mkdirSync10, readFileSync as readFileSync16, rmSync as rmSync3, writeFileSync as writeFileSync10 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync12, readFileSync as readFileSync12, writeFileSync as writeFileSync7 } from "fs";
...
function storyTestListFingerprint(sftddDir, featureId, story_id) {
  ...
  try {
    return createHash("sha1").update(readFileSync12(f)).digest("hex");
  } catch {
    return "";
  }
}
...
init_esm_shims();
import { readdirSync as readdirSync9, readFileSync as readFileSync15, statSync as statSync5 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync16, readFileSync as readFileSync17, readdirSync as readdirSync11 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync18, readFileSync as readFileSync19, readdirSync as readdirSync12, statSync as statSync6 } from "fs";
...
init_esm_shims();
import { existsSync as existsSync20, readFileSync as readFileSync21, readdirSync as readdirSync13, statSync as statSync7 } from "fs";
...
function storyCycles(sftddDir, featureId, story) {
  ...
  for (const acDir of readdirSync14(base)) {
    ...
    for (const f of readdirSync14(dir)) {
      ...
      try {
        out.push(JSON.parse(readFileSync22(join23(dir, f), "utf8")));
      } catch {
      }
    }
  }
...
init_esm_shims();
import { existsSync as existsSync22, readFileSync as readFileSync23, writeFileSync as writeFileSync14, mkdirSync as mkdirSync14, rmSync as rmSync7 } from "fs";
var SMELL_FOR_OWNER = {
...
init_esm_shims();
import { existsSync as existsSync23, readFileSync as readFileSync24, writeFileSync as writeFileSync15, mkdirSync as mkdirSync15, readdirSync as readdirSync15 } from "fs";
function uniq(xs) {
...
async function execute(step, ctx, deps) {
  ...
  for (const spec of step.outputs(action)) {
    ...
    const abs = producedPaths.find((p) => p.endsWith(rel)) ?? join25(workspaceDir, rel);
    if (!existsSync25(abs)) {
...
init_esm_shims();
import { readFileSync as readFileSync26, writeFileSync as writeFileSync16, existsSync as existsSync26, appendFileSync, mkdirSync as mkdirSync16 } from "fs";
...
function formatAgentReport(args) {
  const logPath = join26(args.workspaceDir, args.logFile ?? "agent-log.jsonl");
  let reportJson;
  let source;
  if (args.reportText !== void 0) {
    ...
    const reportPath = join26(args.workspaceDir, args.reportFile ?? ".agent-report.json");
    source = reportPath;
    ...
    reportJson = readFileSync26(reportPath, "utf8");
  }
  let raw;
  try {
  ...
  if (existsSync26(logPath)) {
    ...
    mkdirSync16(dirname14(logPath), { recursive: true });
    writeFileSync16(logPath, payload);
  }
...
function resolveInputsFromWorkspace(manifest, workspaceDir) {
  ...
  for (const input of manifest.inputs) {
    ...
    const p = join27(workspaceDir, file);
    if (!existsSync27(p)) return { missing: input.id };
    out[input.id] = readFileSync27(p, "utf8");
  }
  return out;
}
...
function layDownKitAgents(workspaceDir, kitDir = process.cwd()) {
  const src = join28(kitDir, "skills", "consort", "agents");
  if (!existsSync28(src)) throw new Error(`layDownKitAgents: kit agents dir not found at ${src}`);
  const dest = join28(workspaceDir, ".claude", "agents");
...
async function runIntegrationChain(config) {
  ...
  const workspaceDir = mkdtempSync(join28(tmpdir(), "integration-chain-"));
  mkdirSync17(join28(workspaceDir, ".sftdd"), { recursive: true });
  ...
  const runnerDeps = {
    ...
    cfg: {
      ...
      sftddDir: join28(workspaceDir, ".sftdd"),
