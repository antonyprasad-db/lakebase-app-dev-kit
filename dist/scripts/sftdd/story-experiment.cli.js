...
async function cutExperiment(args, deps = {}) {
  ...
  writeFileSync3(join4(dir, "branch.txt"), branchId);
  writeFileSync3(
    join4(dir, "notes.md"),
...
`
  ...
  writeFileSync3(join4(dir, "outcomes.json"), JSON.stringify(outcomes, null, 2) + "\n");
  writeFileSync3(
    join4(dir, "timeline.json"),
    JSON.stringify(
...
async function deleteExperiment(args) {
  ...
  if (deleteBranchToo) {
    const branchId = readFileSync4(join4(dir, "branch.txt"), "utf8").trim();
...
var AlembicAdapter = {
   ...
   */
  detect(projectDir) {
    if (fs9.existsSync(path6.join(projectDir, "alembic.ini"))) return true;
    if (fs9.existsSync(path6.join(projectDir, "migrations", "env.py"))) return true;
    if (fs9.existsSync(path6.join(projectDir, "alembic", "env.py"))) return true;
    return false;
  },
  async apply(args) {
...
var FlywayAdapter = {
  ...
  detect(projectDir) {
    return fs10.existsSync(path8.join(projectDir, "pom.xml"));
  },
  async apply(args) {
...
var KnexAdapter = {
   ...
   */
  detect(projectDir) {
    return KNEXFILE_VARIANTS2.some((name) => fs12.existsSync(path10.join(projectDir, name)));
  },
  async apply(args) {
