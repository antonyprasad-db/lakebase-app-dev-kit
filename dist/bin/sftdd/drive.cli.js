...
function resolveSchemaDir() {
  const candidates = [
    join6(__dirname, "schemas"),
    // dist: inlined consumer sits beside the copied schemas
    join6(__dirname, "..", "..", "..", "scripts", "sftdd", "schemas")
    // source: consort/orchestrator/validators -> repo root
  ];
  return candidates.find((d) => existsSync6(d)) ?? candidates[0];
}
...
function assessMarkerWritten(producedPath) {
  const sup = join24(producedPath, "superseded-tests.json");
  const reg = join24(producedPath, "regression-assessment.json");
  const hasSup = existsSync28(sup);
  const hasReg = existsSync28(reg);
  if (!hasSup && !hasReg) {
...
var Step = class {
  constructor(manifest, agent, exists) {
    ...
    this.agent = agent;
    this.exists = exists ?? existsSync29;
  }
  manifest;
  agent;
  exists;
  ...
  async run(provided) {
    ...
    for (const spec of this.outputs(action)) {
      ...
      const p = join25(rootFor(spec.channel), rel);
      if (this.exists(p)) {
...
var AGENT_CONFIG_REL = join26(".lakebase", "agent-config.json");
...
// consort/config/sftdd-config-file.ts
var SFTDD_CONFIG_REL = join27(".lakebase", "sftdd-config.json");
var LEGACY_TDD_CONFIG_REL = join27(".lakebase", "tdd-config.json");
...
function writeSftddConfig(projectDir, config, opts) {
  const f = join27(projectDir, TDD_CONFIG_REL);
  if (existsSync31(f) && !opts?.force) return false;
  mkdirSync16(dirname9(f), { recursive: true });
  writeFileSync18(f, JSON.stringify(config, null, 2) + "\n");
  return true;
}
...
function resolveKitRoot() {
  const candidates = [
    path7.resolve(__dirname2, "../.."),
    path7.resolve(__dirname2, "../../..")
  ];
  for (const c of candidates) {
    if (fs11.existsSync(path7.join(c, "package.json")) && fs11.existsSync(path7.join(c, "skills", "consort", "agents"))) {
      return c;
    }
  }
  throw new Error(
...
function kitVersion(root) {
  try {
    return JSON.parse(fs11.readFileSync(path7.join(root, "package.json"), "utf8")).version ?? "";
  } catch {
    return "";
  }
}
...
function resyncAgentsOnKitDrift(projectDir) {
  try {
    ...
    const markerPath3 = path7.join(projectDir, AGENT_SYNC_MARKER);
    let last = "";
    try {
      last = fs11.readFileSync(markerPath3, "utf8").trim();
    } catch {
    }
    if (last === current) return { refreshed: false };
    updateAgents({ projectDir, kitDir: root, force: true });
    fs11.mkdirSync(path7.dirname(markerPath3), { recursive: true });
    fs11.writeFileSync(markerPath3, current + "\n");
  ...
  switch (turn.role) {
    case "spec-author": {
      ...
      if (turn.mode === "breakdown") {
        ...
        if (existsSync35(storiesSrc)) {
          for (const s of readdirSync22(storiesSrc)) {
...
function resolveKitRoot2() {
  ...
  kitRootCache = env && fs12.existsSync(path8.join(env, "package.json")) ? env : path8.resolve(__dirname, "..", "..", "..");
  return kitRootCache;
}
...
function resolveSubstrateRoot() {
  ...
  for (; ; ) {
    ...
    if (fs12.existsSync(path8.join(cand, "package.json"))) {
      substrateRoot = cand;
      return cand;
    }
...
function resolveKitBinJs(bin) {
  if (kitBinMap === null) {
    try {
      const pkg = JSON.parse(fs12.readFileSync(path8.join(resolveKitRoot2(), "package.json"), "utf8"));
      kitBinMap = pkg.bin ?? {};
    } catch {
      kitBinMap = {};
    }
  ...
  if (rel) return path8.join(resolveKitRoot2(), rel);
  const subRoot = resolveSubstrateRoot();
  if (subRoot) {
    if (substrateBinMap === null) {
      try {
        const pkg = JSON.parse(fs12.readFileSync(path8.join(subRoot, "package.json"), "utf8"));
...
function listFilesRel(dir) {
  const out = [];
  const walk2 = (abs, rel) => {
    for (const entry of readdirSync23(abs)) {
      ...
      if (cmd.kind === "verify-artifact") {
        const isPresent = () => cmd.anyOf.some((p) => {
          try {
            ...
            return st.isDirectory() ? fs13.readdirSync(p).length > 0 : true;
          } catch {
            return false;
          }
        });
...
async function performTurnViaExecutor(action, state, routerDeps, cfg, deps) {
  ...
  const inputPath = (source) => {
    if (source.startsWith("story:")) {
      ...
      if (!story) return join34(cfg.sftddDir, rel);
      return join34(storyResolved(cfg.sftddDir, f, story), rel);
    }
    return join34(cfg.sftddDir, source.replace(/^feature:/, ""));
  };
  const executorDeps = {
...
function contextRubric(sftddDir, featureId, story, ac) {
  ...
  if (layers.has("E2E")) {
    try {
      const dg = JSON.parse(fs15.readFileSync(designGuideJson(sftddDir), "utf8"));
...
function storyStubScope(sftddDir, featureId, storyId) {
  try {
    const stub = JSON.parse(fs16.readFileSync(storyJson(sftddDir, featureId, storyId), "utf8"));
    const parts = [
...
function consumeHandback(action, featureId, sftddDir) {
  ...
  let note = "";
  try {
    note = fs16.readFileSync(file, "utf8").trim();
...
function roleTaskBody(action, featureId, uiTrack, sftddDir, build) {
  ...
  switch (action.role) {
    ...
    case "dba": {
      ...
      let contract = "";
      try {
        const arch = JSON.parse(fs16.readFileSync(architectureJson(sftddDir, featureId), "utf8"));
...
function emitNextJson(sftddDir, featureId, projectDir, ctx = {}) {
  try {
    ...
    fs17.mkdirSync(sftddDir, { recursive: true });
    fs17.writeFileSync(path10.join(sftddDir, "next.json"), JSON.stringify(snap, null, 2) + "\n", "utf8");
  } catch {
  }
}
...
function deriveSprintPlanningState(sftddDir, sprint, opts = {}) {
  const proposed = fs18.existsSync(featureProposalsMd(sftddDir));
...
function writeRunConfig(inputs) {
  ...
  try {
    ...
    writeFileSync25(join38(inputs.sftddDir, "run-config.json"), body);
