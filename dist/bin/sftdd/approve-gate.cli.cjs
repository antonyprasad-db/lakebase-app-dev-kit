...
function resolveSchemaDir() {
  const candidates = [
    (0, import_path.join)(__dirname, "schemas"),
    // dist: inlined consumer sits beside the copied schemas
    (0, import_path.join)(__dirname, "..", "..", "..", "scripts", "sftdd", "schemas")
    // source: consort/orchestrator/validators -> repo root
  ];
  return candidates.find((d) => (0, import_fs.existsSync)(d)) ?? candidates[0];
}
...
function writeSprintGates(state, opts = {}) {
  const sftddDir = opts.sftddDir ?? resolveSftddDir();
  (0, import_node_fs.mkdirSync)(sprintDir(sftddDir, state.sprint), { recursive: true });
...
function appendSelectionLog(sftddDir, entry) {
  const logPath = (0, import_path5.join)(sftddDir, "selection-log.md");
