...
function replayBuildTurn(args) {
  ...
  if ((0, import_fs.existsSync)(cyclesSrc)) {
    (0, import_fs.cpSync)(cyclesSrc, cyclesRootDir(sftddDir), {
      recursive: true,
      force: true,
...
function resolveSchemaDir() {
  const candidates = [
    (0, import_path3.join)(__dirname, "schemas"),
    // dist: inlined consumer sits beside the copied schemas
    (0, import_path3.join)(__dirname, "..", "..", "..", "scripts", "sftdd", "schemas")
...
function resolveKitRoot() {
  ...
  for (const c of candidates) {
    if (fs11.existsSync(path6.join(c, "package.json")) && fs11.existsSync(path6.join(c, "skills", "consort", "agents"))) {
      return c;
    }
  }
  throw new Error(
...
function kitVersion(root) {
  try {
    return JSON.parse(fs11.readFileSync(path6.join(root, "package.json"), "utf8")).version ?? "";
  } catch {
    return "";
  }
}
...
function resyncAgentsOnKitDrift(projectDir) {
  try {
    ...
    const markerPath3 = path6.join(projectDir, AGENT_SYNC_MARKER);
    let last = "";
    try {
    ...
    fs11.mkdirSync(path6.dirname(markerPath3), { recursive: true });
...
function replayDesignTurn(args) {
  ...
  const tf = (0, import_path11.join)(featuresDir(sftddDir), featureId);
  switch (turn.role) {
    case "spec-author": {
      if (turn.mode === "propose") {
        return cp((0, import_path11.join)(replayDir, "planning", "feature-proposals.md"), (0, import_path11.join)(sftddDir, "planning", "feature-proposals.md"));
      }
    ...
    case "ux-designer": {
      let ok = cp((0, import_path11.join)(replayDir, "design", "design-guide.json"), (0, import_path11.join)(sftddDir, "design", "design-guide.json"));
      cp((0, import_path11.join)(replayDir, "design", "design-guide.md"), (0, import_path11.join)(sftddDir, "design", "design-guide.md"));
      cp((0, import_path11.join)(replayDir, "design", "ia.md"), (0, import_path11.join)(sftddDir, "design", "ia.md"));
      return ok;
    }
    default:
      return false;
...
function restoreReflectVerdict(args) {
  ...
  return cp(
    ...
    (0, import_path11.join)(featuresDir(sftddDir), featureId, "stories", story, "reflect-verdict.json")
  );
}
...
function resolveKitRoot2() {
  ...
  kitRootCache = env && fs12.existsSync(path7.join(env, "package.json")) ? env : path7.resolve(__dirname, "..", "..", "..");
  return kitRootCache;
}
...
function resolveSubstrateRoot() {
  ...
  for (; ; ) {
    ...
    if (fs12.existsSync(path7.join(cand, "package.json"))) {
      substrateRoot = cand;
      return cand;
    }
...
function resolveKitBinJs(bin) {
  if (kitBinMap === null) {
    try {
      const pkg = JSON.parse(fs12.readFileSync(path7.join(resolveKitRoot2(), "package.json"), "utf8"));
      kitBinMap = pkg.bin ?? {};
    } catch {
      kitBinMap = {};
    }
  ...
  if (subRoot) {
    if (substrateBinMap === null) {
      try {
        const pkg = JSON.parse(fs12.readFileSync(path7.join(subRoot, "package.json"), "utf8"));
...
function malformedSiblingRoot(projectDir) {
  ...
  return `${(0, import_node_path15.dirname)(p)}-${(0, import_node_path15.basename)(p)}`;
}
function listFilesRel(dir) {
  const out = [];
  const walk2 = (abs, rel) => {
    for (const entry of (0, import_node_fs12.readdirSync)(abs)) {
      const childAbs = (0, import_node_path15.join)(abs, entry);
      const childRel = rel ? (0, import_node_path15.join)(rel, entry) : entry;
      if ((0, import_node_fs12.statSync)(childAbs).isDirectory()) walk2(childAbs, childRel);
...
function relocateStrayDesignArtifacts(projectDir) {
  ...
  for (const artRoot of [".sftdd", ".tdd"]) {
    ...
    const realRoot = (0, import_node_path15.join)(projectDir, artRoot);
    (0, import_node_fs12.mkdirSync)(realRoot, { recursive: true });
    (0, import_node_fs12.cpSync)(strayRoot, realRoot, { recursive: true, force: true });
    (0, import_node_fs12.rmSync)(strayRoot, { recursive: true, force: true });
  }
  try {
    if ((0, import_node_fs12.readdirSync)(sibling).length === 0) (0, import_node_fs12.rmSync)(sibling, { recursive: true, force: true });
  } catch {
  }
...
async function performTurnViaExecutor(action, state, routerDeps, cfg, deps) {
  ...
  const inputPath = (source) => {
    if (source.startsWith("story:")) {
      ...
      if (!story) return (0, import_node_path16.join)(cfg.sftddDir, rel);
      return (0, import_node_path16.join)(storyResolved(cfg.sftddDir, f, story), rel);
    }
    return (0, import_node_path16.join)(cfg.sftddDir, source.replace(/^feature:/, ""));
  };
  const executorDeps = {
...
function emitNextJson(sftddDir, featureId, projectDir, ctx = {}) {
  try {
    ...
    fs17.writeFileSync(path9.join(sftddDir, "next.json"), JSON.stringify(snap, null, 2) + "\n", "utf8");
  } catch {
  }
}
