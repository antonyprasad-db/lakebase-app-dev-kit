...
// bin/sftdd/optimize-apply.cli.ts
...
function readRecordedCandidate(experimentsDir, handoff, candidateId) {
  const candDir = join2(experimentsDir, handoff, candidateId);
  if (!existsSync2(candDir)) throw new Error(`optimize-apply: no recorded candidate at ${candDir} (run the sweep first)`);
  ...
  const file = join2(candDir, trials[0], "candidate.json");
  if (!existsSync2(file)) throw new Error(`optimize-apply: missing ${file}`);
...
async function main() {
  ...
  const experimentsDir = join2(projectDir, "experiments");
