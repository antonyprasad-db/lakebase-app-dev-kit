...
1b. **Emit the architectural FITNESS tests.** Walk `architecture.json` (`layers`, `service_backed`, ORM-only, config-in-env, each accepted NFR budget) and add >=1 `kind:"fitness"` item per architectural constraint the story touches: the layering contract (boundary must not import the DB session; persistence only in the repository), ORM-only persistence, config-from-env, and any NFR budget. A behavior test passing while a fitness function is RED is NOT done (see [test-strategy](../references/test-strategy.md)). A service-backed feature with no fitness item HARD-FAILS Gate 3.
   - **A COMPOUND defense needs ONE fitness item per named assertion, not one for the pair.** When a constraint's or NFR's defense names MORE THAN ONE thing , the giveaway is an `and` / a `+` / a comma joining two checkable claims (e.g. "defended by the layering AND boundary-returns-JSON fitness test", "reversibility PLUS data-preservation", "rejects nulls AND enforces the FK") , emit a SEPARATE `kind:"fitness"` item for EACH half, not a single item that covers only one. Read the constraint's full description and enumerate every distinct assertion it promises; a test list that covers one conjunct and drops the other is exactly the half-coverage the reflect gate raises as `reflect-testlist-defect`. Concretely: a layering NFR that says "boundary does not import the DB session AND returns JSON only" is TWO items (a no-DB-import layering test and a boundary-returns-JSON-never-HTML test); a migration NFR that says "reversible AND preserves data" is TWO items (per 1c below). If in doubt whether a phrase is one assertion or two, split it, an extra fitness item is cheap; a missing one hard-halts the gate.
1c. **Cover every persistence invariant with a real-branch test.** Walk `architecture.json` `persistence_invariants[]` (the architect's declared DB contract , unique/FK/cascade/NOT NULL/CHECK/transactional/migration-reversible). For EACH, add a `kind:"fitness"` item with `invariant_id` set to that invariant's id, whose test exercises it DIRECTLY against the branch database (a real DB session, never a mock): verify the MIGRATION realized the guarantee (a duplicate insert raises IntegrityError, a bad row is rejected by a NOT NULL/CHECK, a down-then-up migration round-trips) and that the repository honors it. Do NOT write a test of the ORM's generic add/commit/query round-trip , that tests the library, not your schema. Branch-per-feature exists to make this cheap. A feature that declares invariants but leaves one uncovered HARD-FAILS Gate 3 (checkPersistenceCoverage). **A service does not always mean a database:** a `service_backed` feature that persists nothing declares NO `persistence_invariants[]`, and then there is simply nothing to cover here (this step is a no-op) , do not invent a DB test for a service that has no database. The layering fitness item in 1b still applies to every service-backed feature.
   ...
   - **A migration that carries DATA needs TWO distinct fitness tests: reversibility AND data-preservation.** The schema round-trip above proves the migration REVERSES; it does NOT prove existing data SURVIVES it (a downgrade/upgrade on an EMPTY branch with nothing seeded never exercises data survival, which is exactly the half-coverage the reflect gate flags as `reflect-testlist-defect`). Whenever the story's migration alters a table that holds or will hold rows (an additive column, an expand/contract split, a backfill) AND the architecture declares a data-durability NFR (or a `migration-reversible` invariant on a populated table), emit a SECOND `kind:"fitness"` item, distinct from the reversibility item, that SEEDS representative rows BEFORE the migration, runs the migration, then asserts those pre-existing rows PERSIST with their expected values afterward. For an expand/contract, assert the old column's values are carried correctly into the new columns and survive the down-then-up round-trip. Seed idempotently (next bullet) and mark it `@pytest.mark.migration`. The reversibility test and the data-preservation test are SEPARATE list items: covering only one is the `reflect-testlist-defect` the gate raises. If a data-durability NFR is declared, BOTH must appear, mapped to it.
