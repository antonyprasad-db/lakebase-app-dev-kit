...
- For each `ac.json`: ADD `layer` (`API` / `E2E` / `Infra`) + `architectural_notes` (layer rationale, cross-cutting concerns touched, owner module). **AUGMENT the AC in place , preserve EVERY existing field.** The spec-author already wrote `id`, `status`, `given`, `when`, `then`, and (on every AC after the first) an `independence` block (`distinct_from_prior` + `rationale`); you ADD your two fields ON TOP and re-write the file with ALL prior fields intact. NEVER drop `independence` (or any other field) when you re-write the AC , a re-write that omits `independence` HARD-BLOCKS the spec gate (`missing independence determination`) and cannot self-heal. Read the AC, add your fields, write the union , not a fresh object with only your fields.
