...
- `.sftdd/features/<F>/architecture.json` (validated against its schema): the **NFRs** you propose (`nfrs[]`, each `applies_to` the feature or a story id, `hil_status: "proposed"`). NFRs live HERE, not on `feature-spec.json`/`story.json`.
  ...
  - **`layers[].renders_via`** (boundary layer, UI features only): how the UI is delivered. Two first-class paths. **React SPA** , the project ships a `client/` workspace (React + TypeScript + Vite, laid down by the scaffold with layered `api/`/`hooks/`/`components/`/`pages/` and its own Vitest + Playwright harness); the boundary returns JSON only and the SPA renders it, so declare the boundary `renders_via: "react"`. **Server-side templates** , e.g. `"jinja2"` with a `templates/` dir; the boundary returns a rendered template. When the project was scaffolded with a React client (`.sftdd/sftdd-config.json` `project.clientFramework: "react"`, the default for a UI project), DEFAULT to the React SPA: it is the path of least resistance because the client and its test harness already exist. Only choose server-side templates when the product intent explicitly calls for server-rendered pages. Omit `renders_via` for an API/CLI/Infra boundary that returns data, not markup.
  - **`persistence_invariants`** (array, REQUIRED when `service_backed`): the DB-level guarantees the schema enforces , the persistence contract the test-strategist must cover with a real-branch test each. Declare every one the feature relies on: unique/composite keys, foreign keys + cascade rules, NOT NULL / CHECK constraints, any transactional-atomicity boundary, and migration reversibility. Each entry: `{ "id": "PI1-<slug>", "type": "unique"|"foreign_key"|"cascade"|"not_null"|"check"|"transactional"|"migration_reversible", "table": "<table>", "brief": "<the guarantee to verify against the branch>" }`. A service_backed feature that declares NONE hard-blocks the gate (`checkPersistenceCoverage`): it persists data, so it has at least one. These drive DB coverage tied to the schema's own contract (verifying the migration realized each guarantee against the real branch), not the ORM's generic behavior.
