...
- Local dev ref = `sftdd-capture-local`. `pin_local_kit_cache` (examples/replay/lib/pin-local-kit.sh)
...
- `examples/replay/optimize-scenario.sh` — the run wrapper. USE single-handoff (no `--sweep-lane`).
- `examples/replay/archive-optimize-results.sh` — copies a role's results into the committed optimize-results/<handoff>/ (summary.json + report.md + champion-walk.json + per-candidate trials) so metrics survive project teardown.
- `examples/replay/watch-artifacts.sh` — Monitor-friendly artifact watcher.
