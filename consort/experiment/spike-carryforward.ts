...
import { featurePlanJson } from "../../consort/config/consort-paths.js";
...
/**
 * Scan `<consortDir>/spikes/` and return every spike that references
...
export function collectSpikeInputs(args: CollectSpikeInputsArgs): SpikeInput[] {
  const spikesDir = join(args.consortDir, "spikes");
...
/**
 * Persist `spike_inputs` onto `<consortDir>/features/<featureId>/plan.json`
...
export function attachSpikeInputs(args: AttachSpikeInputsArgs): AttachSpikeInputsResult {
  const planPath = featurePlanJson(args.consortDir, args.featureId);
  if (!existsSync(planPath)) {
    throw new Error(
      `attachSpikeInputs: plan.json not found at ${planPath}. Run the design-spec gate first.`
    );
  }
  const candidates = collectSpikeInputs({ consortDir: args.consortDir, featureId: args.featureId });
