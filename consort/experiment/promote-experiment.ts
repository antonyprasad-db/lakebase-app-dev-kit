...
export async function promoteExperiment(args: PromoteArgs): Promise<PromoteResult> {
  ...
  const { consortDir, featureId, storyId, winnerSlug, approverEmail } = args;
  ...
  const winnerOutcome = readOutcomes(consortDir, featureId, storyId, winnerSlug);
  writeOutcomes(consortDir, featureId, storyId, winnerSlug, { ...(winnerOutcome ?? {}), status: "succeeded" });
  ...
  try {
    const feature = readFeature(consortDir, featureId);
    feature.status = "ready-for-review";
    writeFeature(consortDir, feature);
  } catch {
