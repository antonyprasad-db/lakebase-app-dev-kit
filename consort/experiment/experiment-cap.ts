...
export function checkPerExperimentCap(args: CheckPerExperimentCapArgs): CheckPerExperimentCapResult {
  ...
  if (cap.max_wall_clock_minutes && cap.max_wall_clock_minutes > 0) {
    const startedAtMs = readExperimentStartMs(args.consortDir, args.featureId, args.storyId, args.experimentSlug);
...
export function recordExperimentCap(args: RecordExperimentCapArgs): ExperimentOutcomes {
  const outcomes = readOutcomes(args.consortDir, args.featureId, args.storyId, args.experimentSlug);
  if (!outcomes) {
    throw new Error(
  ...
  writeOutcomes(args.consortDir, args.featureId, args.storyId, args.experimentSlug, outcomes);
  return outcomes;
}
...
function readExperimentStartMs(
  ...
  const file = join(experimentDir(consortDir, featureId, storyId, slug), "timeline.json");
