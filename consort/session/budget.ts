...
export function snapshotBudget(consortDir: string, featureId: string, storyId: string): BudgetSnapshot | null {
  const plan = readPlan(consortDir, featureId, storyId);
  if (!plan) return null;
  ...
  const inUse = experiments.filter((e) => {
    const o: ExperimentOutcomes | null = readOutcomes(consortDir, featureId, storyId, e.experiment_slug);
...
export function canCutAnotherExperiment(consortDir: string, featureId: string, storyId: string): { ok: boolean; reason?: string } {
  const snap = snapshotBudget(consortDir, featureId, storyId);
