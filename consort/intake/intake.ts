...
import { resolveConsortDir, featureRequestMd } from "../../consort/config/consort-paths.js";
...
export function checkIntakePreconditions(args: IntakeCheckArgs = {}): IntakeCheckResult {
  ...
  const projectDir = args.projectDir ?? dirname(consortDir);
  ...
  const required: Array<{ artifact: string; path: string }> = [
    { artifact: "product-overview.md", path: join(consortDir, "product-overview.md") },
    { artifact: "nfrs.md", path: join(consortDir, "nfrs.md") },
  ];
  if (uiTrack) {
    required.push({ artifact: "design-brief.md", path: join(consortDir, "design", "design-brief.md") });
  }
  if (args.featureId) {
