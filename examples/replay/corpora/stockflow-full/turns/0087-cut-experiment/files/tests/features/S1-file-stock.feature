Feature: File stock (S1-file-stock)
  ...
  # T16 – AC2: inventory_code round-trip (S2: combined code replaces batch_number/serial_number)
  Scenario: GET a record filed with an inventory_code returns it exactly as filed
    Given no stock exists for a unique SKU and location
    When the client POSTs a stock record with that SKU, location, quantity, and inventory_code "LOT-42-SN-007"
    Then the response indicates the record was created
    And a subsequent GET for that SKU and location returns inventory_code "LOT-42-SN-007"
