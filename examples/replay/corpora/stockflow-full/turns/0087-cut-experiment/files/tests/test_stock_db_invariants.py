...
def test_stock_migration_is_reversible():
    ...
    # S2 is now head: inventory_code is restored; batch_number/serial_number are absent.
    ...
    for absent_col in ("batch_number", "serial_number"):
        assert absent_col not in cols, (
            f"Column '{absent_col}' must not be present at S2 head after migration round-trip"
        )
