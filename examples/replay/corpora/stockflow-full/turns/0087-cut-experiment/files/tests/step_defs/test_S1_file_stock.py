...
@when(
    'the client POSTs a stock record with that SKU, location, quantity, and inventory_code "LOT-42-SN-007"',
    target_fixture="ctx",
)
...
@then('a subsequent GET for that SKU and location returns inventory_code "LOT-42-SN-007"')
def then_get_returns_inventory_code(ctx):
    ...
    assert resp.status_code == 200, f"GET failed: {resp.status_code} {resp.text}"
    data = resp.json()
    assert data.get("inventory_code") == "LOT-42-SN-007", (
        f"expected inventory_code 'LOT-42-SN-007', got: {data.get('inventory_code')!r}"
    )
