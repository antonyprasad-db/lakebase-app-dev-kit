...
- Add `batch_number` and `serial_number` as first-class columns on the stock record, then backfill them from the existing `inventory_code`. `location` is already its own column (part of UNIQUE(sku, location)), so this iteration does NOT recreate a location column. It extracts only batch and serial.
- Once the split columns are backfilled and validated, the combined `inventory_code` column MUST be dropped from the stock table: the end-state schema no longer carries `inventory_code`. This drop is a distinct outcome — it can be built and verified independently of adding the new columns (you can add + backfill batch/serial and still have `inventory_code` present, so dropping it is its own separately-breakable change). Reversibility is provided by a paired down-migration that reconstructs `inventory_code` on rollback, NOT by leaving the column in place.
