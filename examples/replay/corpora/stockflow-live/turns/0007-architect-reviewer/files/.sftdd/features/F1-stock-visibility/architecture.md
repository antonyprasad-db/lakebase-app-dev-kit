...
UI rendering: the project was scaffolded with a React client
(`clientFramework: "react"`) and R5 requires a single-page app, so the boundary
returns JSON only (`renders_via: "react"`) and the existing `client/` workspace
(Vite + Vitest + Playwright) renders it. No server-side templates.

## Layer assignments (S1-file-stock-record)

- AC1 file-new-record-shows-confirmation , **E2E** (SPA form -> boundary -> service -> repository -> DB; confirmation echo rendered client-side)
- AC2 filed-record-retrievable , **E2E** (durable persistence read-back through the full read path against the branch)
- AC3 refile-same-pair-no-duplicate , **E2E** (write-time collision resolution: service upsert + DB unique constraint, single row survives)
- AC4 refile-same-pair-no-error-page , **E2E** (graceful outcome: refile resolves to a 2xx confirmation, never an error page)
...
- Repository pattern isolates SQLAlchemy in `app/repositories/`; the service
  depends on the repository interface, keeping the ORM out of business logic.
...
- Stock record modeled as a single-responsibility aggregate in
  ...
  for V1 (splitting is a later feature).
...
- Upsert-on-collision (AC3/AC4) is a deliberate semantic: a refile OVERWRITES the
  ...
  record-vs-adjust to differ, this write boundary needs revisiting (adjustment is
  a separate feature).
...
- Recording stock at an unknown location is created implicitly for V1; if
  location master data is later introduced, the unique-constraint scope and write
  path change.

## Decisions (for PO at Gate 2)

1. **Refile semantics** , recommend last-write-wins upsert (overwrite quantity +
   tracking code), matching AC3 (single row holds the new values) and AC4 (no
   error page). Record-vs-adjust distinction deferred to the adjustment feature.
2. **Unknown-location handling** , recommend create implicitly for
   warehouse-floor speed; revisit if location master data arrives.
...
verified through the SPA/E2E flow against the branch (AC1 confirmation echo, AC2
read-back persistence, AC3 single-row upsert, AC4 graceful 2xx confirmation).
Each `persistence_invariant` (PI1 unique, PI2 NOT NULL, PI3 CHECK quantity >= 0,
PI4 migration reversibility) gets a real-branch test tied to the schema's own
contract. Layering, config-in-env, and the JSON/SPA boundary are enforced by the
