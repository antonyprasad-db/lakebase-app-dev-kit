...
def test_par_level_migration_reverses_and_preserves_pre_existing_rows(db_session):
    ...
    try:
        ...
        row = db_session.execute(
            text(
                "SELECT quantity, inventory_code, par_level FROM stock_records "
        ...
        assert row[0] == 11
        assert row[1] == "LOT-T22"
        assert row[2] is None, "par_level must be NULL for a row filed before this migration existed"
    finally:
