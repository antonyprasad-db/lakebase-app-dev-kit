...
def test_alembic_env_resolves_database_url_with_no_hardcoded_dsn():
    """T13 (S1-split-code-migration, AC1-conforming-code-splits-to-batch-and-serial):
    the Alembic migration environment resolves the branch database connection
    from the injected DATABASE_URL env var (connecting to databricks_postgres)
    via app.database's shared, env-driven resolver, with no hardcoded DSN."""
    env_file = REPO_ROOT / "alembic" / "env.py"
    assert env_file.is_file(), f"{env_file} must exist"
    source = env_file.read_text()
    assert "from app.database import" in source and "resolved_url" in source, (
        f"{env_file} must resolve its connection through app.database's "
        "env-driven resolved_url()/make_engine(), not a DSN of its own"
    )
    for scheme in _DSN_SCHEMES:
        assert scheme not in source, (
            f"{env_file} hardcodes a DSN scheme {scheme!r}; the migration "
            "environment must resolve DATABASE_URL via app.database, never a "
            "literal DSN"
        )
