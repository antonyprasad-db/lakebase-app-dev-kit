...
def when_post_stock_level(file_ctx):
    response = _client.post(
        ...
        json={
            ...
            "batch_number": file_ctx["batch_number"],
            "serial_number": file_ctx["serial_number"],
        },
    )
...
def then_row_persisted(file_ctx):
    ...
    try:
        ...
        try:
            ...
            assert row.batch_number == file_ctx["batch_number"]
            assert row.serial_number == file_ctx["serial_number"]
        finally:
            session.close()
    finally:
...
def then_read_back_exact(retrieve_ctx):
    ...
    try:
        ...
        assert data["batch_number"] == retrieve_ctx["batch_number"], (
            f"batch_number mismatch: {data['batch_number']} != {retrieve_ctx['batch_number']}"
        )
        assert data["serial_number"] == retrieve_ctx["serial_number"], (
            f"serial_number mismatch: {data['serial_number']} != {retrieve_ctx['serial_number']}"
        )
    finally:
        _cleanup(sku, location)
...
def when_file_stock_again(upsert_ctx):
    response = _client.post(
        ...
        json={
            ...
            "batch_number": upsert_ctx["batch_number"],
            "serial_number": upsert_ctx["serial_number"],
        },
    )
