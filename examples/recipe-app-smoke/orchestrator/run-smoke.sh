#!/usr/bin/env bash
# TDD-workflow smoke (recipe app).
#
# Same scaffold + flow as the bug-tracker smoke (a Python, UI, 2-tier Lakebase
# project), but the Human Proxy supplies the recipe app's intake + backlog from
# this orchestrator dir (sourced from recipe-app-intake/): product-overview.md,
# nfrs.md, design-brief.md, and the feature-requests/ set (v1..v5 in the
# product-overview's growth-arc order).
#
# Drives the recipe app through 5 evolution iterations (v1..v5), grouped into
# FIVE SPRINTS (one feature each), to exercise the TDD substrate: /plan +
# /design + /build + /deploy + HITL gates (spec / plan / test_list / promote /
# deploy) + per-iteration local tests. The human-proxy replaces a human approver
# so the smoke runs headless.
#
#   v1-recipe-mvp          browse, view, add, edit recipes (the MVP)
#   v2-tag-by-cuisine      classify a recipe by cuisine (first tag kind)
#   v3-search-recipes      search by title/description (read-path extension)
#   v4-draft-recipes       draft vs published visibility
#   v5-review-submissions  review a submission before it joins the collection
#
...
#   sprint-N = vN  (one feature per sprint, in arc order)
...
# Headless, the human-proxy SUPPLIES the recorded backlog (the PO's commitment).
# Each sprint is planned only AFTER the prior sprint's feature has shipped working
# software (its /deploy gate), modeling the feedback loop: the PO folds what they
# saw into the next sprint's request. The sprint backlog is committed to trunk
# (main) so each feature branch (forked from main HEAD by /design Step 0)
...
#                        (v1-recipe-mvp .. v5-review-submissions). Useful when
#                        iterating on the smoke itself.
#   --project-dir <dir>  override the scaffold target. Default:
#                        $SMOKE_ROOT_DEFAULT/recipe-app-<timestamp>
#   --skip-scaffold      assume the recipe app is already scaffolded; jump
...
SMOKE_ROOT_DEFAULT="${HOME}/code/recipe-app-smoke"
RESUME_AT=""
...
PROJECT_NAME="recipe-app-${RUN_ID}"
PROJECT_DIR=""
SKIP_SCAFFOLD=0
KEEP_ON_FAILURE=1
...
# The recipe app's groomed backlog (the recipe-app-intake/feature-requests set),
# in the product-overview's growth-arc order. Each is one sprint (one /plan ->
# /design -> /build -> /deploy pass), so sprint-N is planned only after
# sprint-(N-1) ships, modeling the PO folding feedback into the next sprint.
ITERATIONS=(
  v1-recipe-mvp
  v2-tag-by-cuisine
  v3-search-recipes
  v4-draft-recipes
  v5-review-submissions
)
...
if [[ "$SKIP_SCAFFOLD" -eq 0 ]]; then
  if [[ "$TIERS" != "2" ]]; then
    ...
    echo "       The recipe app is 2-tier (prod + staging): features fork from" >&2
...
# One feature per sprint, in arc order. Each sprint is planned (run_plan_sprint)
# only after the prior sprint shipped working software (its /deploy gate), so the
# PO's next request can fold in what the last increment revealed (the feedback
# loop). --resume <iter> starts mid-arc; run_sprint honors it via the shared
# `started` flag, so the per-feature loop inside still skips up to RESUME_AT.
_sprint_n=0
for _iter in "${ITERATIONS[@]}"; do
  _sprint_n=$((_sprint_n + 1))
  run_sprint "sprint-${_sprint_n}" "$_iter"
done
