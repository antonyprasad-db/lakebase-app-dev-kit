...
Builds on F1-stock-visibility (the stock table and `inventory_code` it introduced). The proof of correctness is the parent-aware schema diff plus the integrity-probe warning count, run against the paired Lakebase branch (R1, R4). A worked reference for the migration shape lives at `.consort/release/migration-examples/split_inventory_code.sql`.
