...
**Terminal outcome (must verify):** delivered expand-then-contract, so the LAST story is the drop. The final (contract) story physically drops `inventory_code` once `batch_number` + `serial_number` are added and backfilled. "Done" means the parent-aware schema diff shows `inventory_code` **removed** from the stock table, not merely the two new columns added. Retiring the combined column is the point of the refactor; adding the new columns while leaving `inventory_code` in place is an incomplete F6.
