...
def assert_new_record_t1(ctx, db_session):
    ...
    assert row[1] == "IC-001", f"inventory_code mismatch: {row[1]}"
...
def existing_record_t2(db_session):
    ...
    db_session.execute(
        ...
        {"sku": sku, "loc": "BIN-B2", "qty": 5, "ic": "OLD-001"},
    )
    db_session.commit()
...
def assert_updated_record_t2(ctx, db_session):
    ...
    assert row[1] == "NEW-999", f"inventory_code not updated: {row[1]}"
