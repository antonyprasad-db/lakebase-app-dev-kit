...
def test_T9_unique_constraint_sku_location(db_session):
    ...
    db_session.execute(
        ...
        {"sku": sku, "loc": loc, "qty": 1, "ic": "IC-PI1"},
    )
    db_session.commit()
    ...
    with pytest.raises(Exception) as exc_info:
        db_session.execute(
            ...
            {"sku": sku, "loc": loc, "qty": 2, "ic": "IC-PI1-DUP"},
        )
        db_session.commit()
...
@pytest.mark.parametrize(
    ...
    [
        (
            "sku",
            {"loc": "BIN-PI2", "qty": 1, "ic": "IC-PI2"},
        ),
        (
            "location",
            {"sku": "pi2-loc-missing", "qty": 1, "ic": "IC-PI2"},
        ),
        (
            "quantity",
            {"sku": "pi2-qty-missing", "loc": "BIN-PI2", "ic": "IC-PI2"},
        ),
        (
            "inventory_code",
            {"sku": "pi2-ic-missing", "loc": "BIN-PI2", "qty": 1},
        ),
    ],
)
def test_T10_not_null_constraint(db_session, missing_field, insert_kwargs):
    ...
    col_map = {"sku": "sku", "loc": "location", "qty": "quantity", "ic": "inventory_code"}
...
def test_T11_check_quantity_non_negative(db_session):
    ...
    with pytest.raises(Exception) as exc_info:
        db_session.execute(
            ...
            {"sku": sku, "loc": "BIN-PI3", "qty": -1, "ic": "IC-PI3"},
        )
        db_session.commit()
