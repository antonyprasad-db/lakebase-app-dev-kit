...
def given_sku_at_two_locations():
    ...
    records = [
        {"sku": sku, "location": loc_a, "quantity": 10, "inventory_code": "CODE-A"},
        {"sku": sku, "location": loc_b, "quantity": 25, "inventory_code": "CODE-B"},
    ]
    session = SessionLocal()
    try:
        session.execute(
...
def given_sku_with_known_inventory_code():
    sku = _make_sku()
    loc = _make_location()
    inventory_code = f"TRACK-{uuid.uuid4().hex[:8].upper()}"
    session = SessionLocal()
    try:
        session.execute(
            sa.text("DELETE FROM stock_records WHERE sku = :sku"),
        ...
        session.execute(
            ...
            {"sku": sku, "location": loc, "quantity": 5, "inventory_code": inventory_code},
        )
        session.commit()
    except Exception:
        session.rollback()
        ...
        session.close()
    return {"sku": sku, "location": loc, "inventory_code": inventory_code}
...
def then_entries_carry_inventory_code(detail_code_ctx, detail_code_response):
    ...
    try:
        ...
        entry = body[0]
        assert "inventory_code" in entry, (
            f"SKU-detail entry missing 'inventory_code' field: {entry}"
        )
        assert entry["inventory_code"] == detail_code_ctx["inventory_code"], (
            f"inventory_code mismatch: got {entry['inventory_code']!r}, "
            f"expected {detail_code_ctx['inventory_code']!r}"
        )
    finally:
        _cleanup_sku(sku)
...
def given_sku_with_null_par_level():
    ...
    try:
        ...
        session.execute(
            ...
            {"sku": sku, "location": loc, "quantity": 7, "inventory_code": "PAR-TEST"},
        )
        session.commit()
    except Exception:
        session.rollback()
