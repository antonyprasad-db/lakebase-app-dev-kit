...
def test_T14_migration_round_trips():
    ...
    # Required columns must exist (inventory_code was replaced by batch_number + serial_number).
    columns = {col["name"] for col in inspector.get_columns("stock_records")}
    for required in ("sku", "location", "quantity", "batch_number", "serial_number"):
        assert required in columns, (
            f"Column '{required}' missing after migration round-trip (PI5)."
        )
    assert "inventory_code" not in columns, (
        f"inventory_code must not exist after the split migration round-trip (PI5)."
    )
        ...
        try:
            conn.execute(
                sa.text(
                    ...
                    "VALUES ('TEST-MIGCHK', 'LOC-MIGCHK', -1)"
                )
            )
            conn.rollback()
            assert False, (
