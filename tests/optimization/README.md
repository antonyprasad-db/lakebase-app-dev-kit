...
1. **Inputs (the fixtures).** The chain manifests + recorded artifacts these tests replay are
   ...
   (`examples/replay/corpora/*`) and copied into what the isolated tests need. They are copies BY
...
- **The recorded corpora are REGRESSION fixtures first.** The canonical, complete copies live in
  `examples/replay/corpora/*` (stockflow-rerecord etc.). Their primary job: a full **agents-off
  ...
  integrity guards + the manual `replay-scenario.sh`) — filed as **#574**. `examples/replay/corpora/`
...
- `examples/replay/corpora/` (the regression corpus) is untouched.
