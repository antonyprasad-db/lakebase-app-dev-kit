...
import {
  ...
  sprintDir,
  PLAN_GATE_ARTIFACT,
} from "../../consort/gates/sprint-gates";
...
describe("readSprintGates", () => {
  it("returns the default (plan open) when no gates.json exists", () => {
    expect(readSprintGates(SPRINT, { consortDir: tdd })).toEqual(defaultSprintGatesState(SPRINT));
    expect(readSprintGates(SPRINT, { consortDir: tdd }).gates.plan.status).toBe("open");
  });
});
...
describe("approveSprintPlanGate (teeth)", () => {
  it("approves when feature-proposals.md exists + conforms", () => {
    writeProposal();
    const res = approveSprintPlanGate({ sprint: SPRINT, approver: "human-proxy", hitlApproved: true, consortDir: tdd });
    expect(res.ok).toBe(true);
    if (res.ok) expect(res.alreadyApproved).toBe(false);
    expect(readSprintGates(SPRINT, { consortDir: tdd }).gates.plan.status).toBe("approved");
  });
  ...
  it("REFUSES without hitlApproved (the gate is HITL)", () => {
    writeProposal();
    const res = approveSprintPlanGate({ sprint: SPRINT, approver: "human-proxy", hitlApproved: false, consortDir: tdd });
    expect(res.ok).toBe(false);
  });

  it("is idempotent: a second approve is a no-op (alreadyApproved)", () => {
    writeProposal();
    approveSprintPlanGate({ sprint: SPRINT, approver: "human-proxy", hitlApproved: true, consortDir: tdd });
    const again = approveSprintPlanGate({ sprint: SPRINT, approver: "human-proxy", hitlApproved: true, consortDir: tdd });
