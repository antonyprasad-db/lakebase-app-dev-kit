...
import { readGates, writeGates, defaultGatesState } from "../../consort/gates/gates";
...
describe("migrateGatesFromSelectionLog: S7 full backfill", () => {
  it("synthesizes gates.json from a complete approval log", () => {
    ...
    const result = migrateGatesFromSelectionLog({ featureId: FEATURE_ID, consortDir: tdd });
  ...
  it("persists the synthesized state: subsequent readGates returns the same shape", () => {
    makeFeatureDir();
    writeLog(FULL_APPROVAL_LOG);
    migrateGatesFromSelectionLog({ featureId: FEATURE_ID, consortDir: tdd });
