...
function seedFeatureAndAc(
  ...
  const acDir = path.join(consortDir, "features", opts.featureId, "stories", opts.storyId, "acs");
...
function seedExperiment(consortDir: string, featureId: string, slug: string, storyId = "S1"): void {
  const dir = path.join(consortDir, "experiments", featureId, storyId, slug);
...
describe("readAcLayer", () => {
  ...
  beforeEach(() => {
    consortDir = mkTempTdd("readac");
  });
  afterEach(() => rmTempTdd(consortDir));

  it("returns the layer when the AC declares one", () => {
    seedFeatureAndAc(consortDir, { featureId: "F1", storyId: "S1", acId: "AC1", layer: "E2E" });
    expect(readAcLayer(consortDir, "F1", "AC1")).toBe("E2E");
  });

  it("returns undefined when the AC is missing", () => {
    expect(readAcLayer(consortDir, "F1", "AC1")).toBeUndefined();
  });

  it("returns undefined when the AC has no layer field", () => {
    seedFeatureAndAc(consortDir, { featureId: "F1", storyId: "S1", acId: "AC1" });
    expect(readAcLayer(consortDir, "F1", "AC1")).toBeUndefined();
  });

  it("scans across stories to find the AC", () => {
    seedFeatureAndAc(consortDir, { featureId: "F1", storyId: "S2", acId: "AC42", layer: "API" });
    expect(readAcLayer(consortDir, "F1", "AC42")).toBe("API");
  });
});
...
describe("recordRunnerOutcome", () => {
  ...
  beforeEach(() => {
    ...
    seedFeatureAndAc(consortDir, { featureId: "F1", storyId: "S1", acId: "AC1", layer: "E2E" });
    seedExperiment(consortDir, "F1", "exp");
  });
  afterEach(() => rmTempTdd(consortDir));
  ...
  it("bumps outcomes by_tag for the cycle's layer and returns the count", () => {
    ...
    const outcomes = readOutcomes(consortDir, "F1", "S1", "exp")!;
    expect(outcomes.by_tag?.e2e).toEqual({ passed: 1, failed: 0 });
  });
...
describe("writeOutcomes pass-through (regression)", () => {
  it("recordTagRun mutation persists when written through writeOutcomes", () => {
    const consortDir = mkTempTdd("write");
    try {
      seedExperiment(consortDir, "F1", "exp");
      const outcomes = readOutcomes(consortDir, "F1", "S1", "exp")!;
      recordTagRun(outcomes, "infra", false);
      writeOutcomes(consortDir, "F1", "S1", "exp", outcomes);
      const reread = readOutcomes(consortDir, "F1", "S1", "exp")!;
      expect(reread.by_tag?.infra).toEqual({ passed: 0, failed: 1 });
      expect(reread.tests_failed).toBe(1);
    } finally {
      rmTempTdd(consortDir);
    }
  });
});
