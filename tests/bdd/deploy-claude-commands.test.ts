...
describe("deployClaudeCommands", () => {
  ...
  it("design.md drives via consort-drive --only design + names the design roles + the pre/post-hook convention", async () => {
    await deployClaudeCommands(targetDir, { templatesDir: REPO_TEMPLATES });
    const design = fs.readFileSync(path.join(targetDir, ".claude", "commands", "design.md"), "utf8");
    expect(design).toMatch(/consort-drive.*--only design/);
  ...
  it("build.md drives via consort-drive --only build + names navigator/driver (no scrum-master)", async () => {
    await deployClaudeCommands(targetDir, { templatesDir: REPO_TEMPLATES });
    const build = fs.readFileSync(path.join(targetDir, ".claude", "commands", "build.md"), "utf8");
    expect(build).toMatch(/consort-drive.*--only build/);
  ...
  it("sprint.md + spike.md are the Tier-1 orchestrator + the spike entry", async () => {
    ...
    expect(sprint).toMatch(/consort-drive --sprint/);
    expect(sprint).not.toMatch(/scrum-master/);
    expect(spike).toMatch(/consort-spike/);
  });
});
