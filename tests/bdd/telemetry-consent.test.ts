...
//   telemetry_enabled === true && isTTY && !CI && CONSORT_TELEMETRY!=="0".
...
describe("telemetry consent predicate", () => {
  it("emits when all conditions hold (enabled + TTY + no CI/kill)", () => {
  ...
  it("a disabling env wins even when everything else says emit", () => {
    expect(shouldEmitTelemetry({ telemetryEnabled: true, isTTY: true, env: { CONSORT_TELEMETRY: "0" } })).toBe(false);
  });
});
