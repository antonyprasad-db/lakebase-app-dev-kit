...
describe("withdrawGate: S6 spec withdraw cascades to plan + test_list", () => {
  ...
  it("preserves the prior approver + approved_at + artifact_hashes on withdrawn gates", () => {
    makeFeatureDir();
    approve("spec", { "feature-spec.md": "x", "feature-spec.json": "{}" });
    const beforeApprover = readGates(FEATURE_ID, { consortDir: tdd }).gates.spec.approver;
    const beforeHashes = readGates(FEATURE_ID, { consortDir: tdd }).gates.spec.artifact_hashes;
    ...
    const after = readGates(FEATURE_ID, { consortDir: tdd }).gates.spec;
