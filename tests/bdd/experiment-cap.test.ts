...
function seedExperiment(
  ...
  const dir = path.join(consortDir, "experiments", featureId, opts.storyId ?? "S1", slug);
...
describe("checkPerExperimentCap: max_wall_clock_minutes", () => {
  let consortDir: string;
  beforeEach(() => {
    consortDir = mkTempTdd("wallclock");
    seedExperiment(consortDir, "F1", "exp", { cutAt: "2026-06-02T08:00:00.000Z" });
  });
  afterEach(() => rm(consortDir));
...
describe("recordExperimentCap + clearExperimentCap", () => {
  ...
  it("clearExperimentCap removes the capped field; second call is a no-op", () => {
    ...
    clearExperimentCap({ consortDir, featureId: "F1", storyId: "S1", experimentSlug: "exp" });
    expect(readOutcomes(consortDir, "F1", "S1", "exp")?.capped).toBeUndefined();
    // Idempotent.
    expect(() =>
      clearExperimentCap({ consortDir, featureId: "F1", storyId: "S1", experimentSlug: "exp" })
    ).not.toThrow();
...
describe("analyzeForGate populates a default per-experiment cap", () => {
  it("proposed_plan.budget.per_experiment carries default max_cycles + max_wall_clock_minutes", async () => {
    ...
    try {
      fs.mkdirSync(path.join(consortDir, "features", "F1"), { recursive: true });
...
describe("compareExperiments surfaces capped outcomes", () => {
  it("classifies signal as 'capped' and copies the cap onto the experiment row", async () => {
    ...
    const consortDir = mkTempTdd("compare");
    try {
      seedExperiment(consortDir, "F1", "exp-a");
      recordExperimentCap({
        consortDir,
        featureId: "F1",
      storyId: "S1",
      ...
      const report = compareExperiments(consortDir, "F1", "S1");
