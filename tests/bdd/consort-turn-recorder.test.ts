...
describe("recordTurn: per-turn timeline + cumulative .tdd mirror", () => {
  it("records each turn's delta, mirrors .tdd, and keeps an ordered index", () => {
    ...
    const t0 = recordTurn({ recordDir: record, projectDir: proj, consortDir: tdd, step: 0, action: act({ kind: "invoke-role", role: "spec-author", mode: "breakdown", story: "S1" }) });
    ...
    const t1 = recordTurn({ recordDir: record, projectDir: proj, consortDir: tdd, step: 1, action: act({ kind: "invoke-role", role: "driver", buildMode: "green" }) });
    ...
    const t2 = recordTurn({ recordDir: record, projectDir: proj, consortDir: tdd, step: 2, action: act({ kind: "invoke-role", role: "architect-reviewer", story: "S1" }) });
  ...
  it("records deletions + removes them from the cumulative mirror", () => {
    ...
    const t = recordTurn({ recordDir: record, projectDir: proj, consortDir: tdd, step: 1, action: act({ kind: "invoke-role", role: "spec-author" }) });
  ...
  it("seedRecorderBaseline makes turn 0 report only what that turn produced (not pre-existing scaffold)", () => {
    ...
    expect(seedRecorderBaseline({ recordDir: record, projectDir: proj, consortDir: tdd })).toBe(true);
    // Re-seed is a no-op once a baseline exists.
    expect(seedRecorderBaseline({ recordDir: record, projectDir: proj, consortDir: tdd })).toBe(false);
    // Turn 0 produces ONE new artifact.
    writeTdd(tdd, "planning/feature-proposals.md", "# proposals");
    const t = recordTurn({ recordDir: record, projectDir: proj, consortDir: tdd, step: 0, action: act({ kind: "invoke-role", role: "spec-author", mode: "propose" }) });
  ...
  it("does not record the append-only agent-log as a produced artifact", () => {
    ...
    const t = recordTurn({ recordDir: record, projectDir: proj, consortDir: tdd, step: 0, action: act({ kind: "invoke-role", role: "ux-designer" }) });
...
describe("record -> replay round-trip", () => {
  it("recorded-artifacts produced by the recorder is consumable by replayDesignTurn", () => {
    ...
    recordTurn({ recordDir: record, projectDir: proj, consortDir: tdd, step: 0, action: act({ kind: "invoke-role", role: "ux-designer" }) });
