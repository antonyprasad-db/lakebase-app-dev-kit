...
describe("promoteExperiment", () => {
  it("throws when hitlApproved is false", async () => {
    ...
    await expect(
      promoteExperiment({ consortDir: tdd, featureId: "F1", storyId: "S1", winnerSlug: "exp-winner", hitlApproved: false })
  ...
  it("throws when winnerSlug does not exist", async () => {
    ...
    await expect(
      promoteExperiment({ consortDir: tdd, featureId: "F1", storyId: "S1", winnerSlug: "ghost", hitlApproved: true })
