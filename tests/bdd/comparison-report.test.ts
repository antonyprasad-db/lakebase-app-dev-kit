...
describe("writeComparisonReport", () => {
  ...
  it("writes the rendered markdown to features/<F>/comparison-<timestamp>.md and appends a selection-log entry", () => {
    const report = mkReport({});
    const result = writeComparisonReport({ consortDir, featureId: "F1", report });
    ...
    const log = fs.readFileSync(path.join(consortDir, "selection-log.md"), "utf8");
  ...
  it("skipSelectionLog=true skips the breadcrumb append", () => {
    ...
    expect(fs.existsSync(path.join(consortDir, "selection-log.md"))).toBe(false);
  });

  it("appends to an existing selection-log without clobbering prior content", () => {
    const logPath = path.join(consortDir, "selection-log.md");
    fs.writeFileSync(logPath, "# Selection log\n\nPrior entry.\n");
    writeComparisonReport({ consortDir, featureId: "F1", report: mkReport({}) });
