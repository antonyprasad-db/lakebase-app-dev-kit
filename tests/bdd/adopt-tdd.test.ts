...
import { adoptTdd } from "../../consort/lakebase/adopt-consort";
...
describe("adoptTdd: fresh adoption", () => {
  ...
  it("copies the bootstrap tree into .consort/ and reports every file in `added`", () => {
    ...
    expect(fs.existsSync(path.join(repo, ".consort", "product-overview.md"))).toBe(true);
    expect(fs.existsSync(path.join(repo, ".consort", "workflow-state.json"))).toBe(true);
    expect(fs.existsSync(path.join(repo, ".consort", "features", ".gitkeep"))).toBe(true);
  });

  it("refuses without --update when .consort/ already exists", () => {
...
describe("adoptTdd: --update mode", () => {
  ...
  it("reports drift on user-edited files without overwriting them", () => {
    adoptTdd({ projectDir: repo });
    const specPath = path.join(repo, ".consort", "product-overview.md");
  ...
  it("adds files that the project is missing relative to the kit template", () => {
    adoptTdd({ projectDir: repo });
    fs.rmSync(path.join(repo, ".consort", "smells.json"));
    const result = adoptTdd({ projectDir: repo, update: true });
    expect(result.added).toContain("smells.json");
    expect(fs.existsSync(path.join(repo, ".consort", "smells.json"))).toBe(true);
  });
});
...
describe("adoptTdd: --force mode", () => {
  ...
  it("force implies update: it also runs on a project that already has .consort/", () => {
    adoptTdd({ projectDir: repo });
...
describe("adoptTdd: --dry-run mode", () => {
  ...
  it("reports a fresh adoption without writing any files", () => {
    ...
    expect(result.added.length).toBeGreaterThan(0);
    expect(fs.existsSync(path.join(repo, ".consort"))).toBe(false);
  });

  it("reports the same buckets in --update --dry-run mode", () => {
    adoptTdd({ projectDir: repo });
    fs.writeFileSync(path.join(repo, ".consort", "product-overview.md"), "user-edit\n", "utf8");
    const result = adoptTdd({ projectDir: repo, update: true, dryRun: true });
    expect(result.drifted).toContain("product-overview.md");
    // No files modified during dry-run.
    expect(fs.readFileSync(path.join(repo, ".consort", "product-overview.md"), "utf8")).toBe("user-edit\n");
  });
...
describe("adoptTdd: bootstrap-dir override", () => {
  ...
  it("honors an explicit bootstrapDir and skips the auto-locate walk", () => {
    ...
    try {
      ...
      expect(fs.readFileSync(path.join(repo, ".consort", "README.md"), "utf8")).toBe(
        "# fixture-only\n"
      );
    } finally {
      rm(fixture);
