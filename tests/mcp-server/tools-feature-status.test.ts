...
describe("MCP tool: lakebase_feature_status", () => {
  ...
  it("handler defaults consortDir to ./.tdd when omitted", async () => {
    ...
    // omitting consortDir is shape-legal.
    const result = (await tool.handler({
      featureId: FEATURE_ID,
      consortDir: tdd, // still pass so the read succeeds
    })) as FeatureStatusSnapshot;
    expect(result.feature_id).toBe(FEATURE_ID);
  });
  ...
  it("handler rejects when featureId is missing", async () => {
    const tool = findTool("lakebase_feature_status")!;
    await expect(tool.handler({ consortDir: tdd })).rejects.toThrow(/featureId/);
  });
