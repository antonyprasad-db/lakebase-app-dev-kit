...
function stage(mode: "clean" | "clean_patched" | "corrupt" | "minimal_clean"): { proj: string; consortDir: string } {
  const proj = mkdtempSync(join(tmpdir(), `live-reflect-${mode}-`));
  const consortDir = join(proj, ".tdd");
  ...
  const featDst = join(consortDir, "features", FEATURE);
  ...
  if (mode === "minimal_clean") {
    ...
    writeFileSync(storyTestListJson(consortDir, FEATURE, STORY), JSON.stringify({
  ...
  if (mode === "corrupt") {
    writeFileSync(
      join(acsDir(consortDir, FEATURE, STORY), "AC9-blank-submits-ok.json"),
      JSON.stringify(
        {
  ...
  const acs = new Set(storyAcIds(consortDir, FEATURE, STORY));
  const items = (master.items ?? []).filter((it) => it.ac_id && acs.has(it.ac_id)) as Array<Record<string, unknown>>;
  writeFileSync(storyTestListJson(consortDir, FEATURE, STORY), JSON.stringify({ feature_id: FEATURE, story_id: STORY, items }, null, 2));
  return { proj, consortDir };
}

function runReflect(proj: string, consortDir: string) {
  const cmds = commandsForAction(
    { kind: "invoke-role", role: "navigator", story: STORY, buildMode: "reflect" },
    { projectDir: proj, consortDir, featureId: FEATURE, runner: { async run() {} }, modelForRole: () => "sonnet" },
  );
  ...
  return { verdict: readReflectVerdict(consortDir, FEATURE, STORY), hits: recordReflectionGate(consortDir, FEATURE, STORY) };
}
...
describe.skipIf(!process.env.RUN_LIVE_REFLECT)("LIVE reflection gate on recorded stockflow F1 (real Navigator turn)", () => {
  it("no-false-positive control: a hand-built, defect-free design passes in one round, no smell", () => {
    const { proj, consortDir } = stage("minimal_clean");
    const { verdict, hits } = runReflect(proj, consortDir);
    // eslint-disable-next-line no-console
  ...
  it("detection: an injected contradictory AC is caught + routed to the spec author", () => {
    const { proj, consortDir } = stage("corrupt");
    const { verdict, hits } = runReflect(proj, consortDir);
    // eslint-disable-next-line no-console
