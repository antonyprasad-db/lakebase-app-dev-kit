...
if [ ! -x "$BIN" ]; then
  ALT="${WORK_TREE:-$PWD}/node_modules/@databricks-solutions/consort/dist/scripts/lakebase/ci-resolve-branch.cli.js"
  if [ ! -f "$ALT" ]; then
    echo "set-production-db-secrets: consort not installed. Run 'npm install'." >&2
    exit 1
  fi
  BIN="node $ALT"
fi
