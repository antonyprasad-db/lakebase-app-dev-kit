...
if [ ! -x "$BIN" ]; then
  ALT="$WORK_TREE/node_modules/@databricks-solutions/consort/dist/scripts/lakebase/schema-diff.cli.js"
  if [ -f "$ALT" ]; then
    BIN="node $ALT"
    ...
    echo "prepare-schema-diff: consort not resolvable (no node_modules, dist, or scripts/lk shim)." >&2
    exit 1
  fi
fi
