...
| `feature-claimed` | `lakebase-scm-prepare-pr` | `pr-ready` | refuses a dirty CODE working tree (ignores the runtime-artifact dirs `RUNTIME_ARTIFACT_IGNORE`: `.consort/`, `.sftdd/`, `.tdd/`, `.lakebase/`, `.claude/agent-memory/`); pushes the branch + opens the PR |
