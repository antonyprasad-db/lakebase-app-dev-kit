...
2. **[done] Substrate.** Landed isolated in `.sftdd/features/<F>/pipeline.json` (NOT overloading the per-feature `gates.json`):
   - 2a pipeline state + single-lane FIFO ready queue (`story-pipeline.ts`, `consort-pipeline` CLI, schema).
