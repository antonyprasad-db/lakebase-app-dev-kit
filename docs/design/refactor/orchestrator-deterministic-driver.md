...
Status: SHIPPED. The deterministic driver (`consort-drive`, with
...
2. Observability. Its `phase.start` / `handoff` / `phase.end` events are
   prose-instructed `consort-log` calls, which haiku drops entirely (a
...
(`consort-drive`) that owns sequencing, and uses the LLM ONLY for the role
...
drive(feature):
  loop:
    ...
    switch action.kind:
      ...
      SURFACE_GATE  -> consort-pipeline surface       # Human Proxy approves headless
      APPROVE_GATE  -> consort-pipeline approve-gate
      DISPATCH      -> consort-pipeline dispatch + regenerate per-story test list
      EXPERIMENT    -> consort-experiment cut/merge/discard
      DEPLOY        -> consort-deploy
      DONE          -> return
...
| log          | `consort-log` (code)          | same (code)                              |
...
slash commands, each a thin invocation of `consort-drive` scoped to a phase
...
between phases. CLI scope: `consort-drive --sprint <name>` owns the
...
| `/spike <slug> [--for <feature>]` | not a driver phase: `consort-spike cut` | throwaway branch + carry-forward notes | `/design <feature>` |
...
`/spike` command + `consort-spike` CLI wrap the existing `spike.ts` +
...
- **Phase 5, `/spike`.** `consort-spike` CLI wrapping `spike.ts`
...
3. Real wiring + CLI. `consort-drive --feature <id>` wires the real effects
   (claude -p --agent, the kit CLIs). Deterministic logging built in.
4. Smoke adoption. The smoke drives via `consort-drive` instead of
